...
struct gid_metadata {
	...
	const size_t bitmap_size;  // Cached numebr of uint32_t's used to map gid_max. Use gid_size_to_words to compute

	// internal usage to ensure that GIDs get delegated linearly before wrapping around back to 0
	size_t _cur_val;
	mutex_t _lock;
};
