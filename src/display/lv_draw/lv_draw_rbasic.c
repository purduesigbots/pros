...
void lv_rfill(const lv_area_t *cords_p, const lv_area_t *mask_p,
  ...
  if (union_ok != false) {
    lv_disp_fill(masked_area.x1, masked_area.y1, masked_area.x2, masked_area.y2,
                 color);
  }
}
...
void lv_rletter(const lv_point_t *pos_p, const lv_area_t *mask_p,
  ...
  static uint8_t bpp1_opa_table[2] = {
      0, 255}; /*Opacity mapping with bpp = 1 (Just for compatibility)*/
  static uint8_t bpp2_opa_table[4] = {0, 85, 170,
                                      255}; /*Opacity mapping with bpp = 2*/
  static uint8_t bpp4_opa_table[16] = {
  ...
  if (pos_p->x + letter_w < mask_p->x1 || pos_p->x > mask_p->x2 ||
      pos_p->y + letter_h < mask_p->y1 || pos_p->y > mask_p->y2)
    return;

  lv_coord_t col, row;
  uint8_t col_bit;
  uint8_t col_byte_cnt;
  uint8_t width_byte_scr = letter_w >> 3; /*Width in bytes (on the screen
                                             finally) (e.g. w = 11 -> 2 bytes
                                             wide)*/
  if (letter_w & 0x7)
    width_byte_scr++;
  ...
  lv_coord_t col_end =
      pos_p->x + letter_w <= mask_p->x2 ? letter_w : mask_p->x2 - pos_p->x + 1;
  lv_coord_t row_start = pos_p->y >= mask_p->y1 ? 0 : mask_p->y1 - pos_p->y;
  lv_coord_t row_end =
      pos_p->y + letter_h <= mask_p->y2 ? letter_h : mask_p->y2 - pos_p->y + 1;
  ...
  for (row = row_start; row < row_end; row++) {
    ...
    for (col = col_start; col < col_end; col++) {
      ...
      if (letter_px != 0) {
        lv_rpx(pos_p->x + col, pos_p->y + row, mask_p,
               lv_color_mix(color, letter_bg_color,
                            bpp == 8 ? letter_px : bpp_opa_table[letter_px]),
               LV_OPA_COVER);
      }
 ...
 */
void lv_rletter_set_background(lv_color_t color) { letter_bg_color = color; }
 ...
 * @param opa opacity of the map (ignored, only for compatibility with
 * 'lv_vmap')
 * @param chroma_keyed true: enable transparency of LV_IMG_LV_COLOR_TRANSP color
 * pixels
 * @param alpha_byte true: extra alpha byte is inserted for every pixel (not
 * supported, only l'v_vmap' can draw it)
...
void lv_rmap(const lv_area_t *cords_p, const lv_area_t *mask_p,
  ...
  if (alpha_byte)
    return; /*Pixel level opacity i not supported in real map drawing*/
  ...
  if (recolor_opa == LV_OPA_TRANSP && chroma_key == false) {
    ...
    for (row = masked_a.y1; row <= masked_a.y2; row++) {
      lv_disp_map(masked_a.x1, row, masked_a.x1 + mask_w, row,
                  (lv_color_t *)map_p);
    ...
    for (row = masked_a.y1; row <= masked_a.y2; row++) {
      for (col = masked_a.x1; col <= masked_a.x2; col++) {
        lv_color_t *px_color =
            (lv_color_t
                 *)&map_p[(uint32_t)(col - masked_a.x1) * sizeof(lv_color_t)];

        if (chroma_key && chroma_key_color.full == px_color->full)
          continue;

        if (recolor_opa != LV_OPA_TRANSP) {
          lv_color_t recolored_px =
              lv_color_mix(recolor, *px_color, recolor_opa);
