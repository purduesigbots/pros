...
void lv_rletter(const lv_point_t* pos_p, const lv_area_t* mask_p, const lv_font_t* font_p, uint32_t letter,
	...
	if (pos_p->x + letter_w < mask_p->x1 || pos_p->x > mask_p->x2 || pos_p->y + letter_h < mask_p->y1 ||
	    pos_p->y > mask_p->y2)
		return;
		...
		for (col = col_start; col < col_end; col++) {
			...
			if (letter_px != 0) {
				lv_rpx(pos_p->x + col, pos_p->y + row, mask_p,
				       lv_color_mix(color, letter_bg_color, bpp == 8 ? letter_px : bpp_opa_table[letter_px]), LV_OPA_COVER);
			}
