 ...
 *  STATIC PROTOTYPES
 **********************/
static void sw_mem_blend(lv_color_t *dest, const lv_color_t *src,
                         uint32_t length, lv_opa_t opa);
static void sw_color_fill(lv_area_t *mem_area, lv_color_t *mem,
 ...
 */
void lv_vpx(lv_coord_t x, lv_coord_t y, const lv_area_t *mask_p,
...
void lv_vletter(const lv_point_t *pos_p, const lv_area_t *mask_p,
  ...
  vdb_buf_tmp +=
      ((pos_p->y - vdb_p->area.y1) * vdb_width) + pos_p->x - vdb_p->area.x1;
  ...
  for (row = row_start; row < row_end; row++) {
    ...
    for (col = col_start; col < col_end; col++) {
      ...
      if (letter_px != 0) {
        if (opa == LV_OPA_COVER) {
          *vdb_buf_tmp =
              lv_color_mix(color, *vdb_buf_tmp,
                           bpp == 8 ? letter_px : bpp_opa_table[letter_px]);
        } else {
          *vdb_buf_tmp = lv_color_mix(
              color, *vdb_buf_tmp,
              bpp == 8
  ...
  /*In the other cases every pixel need to be checked one-by-one*/
    ...
    for (row = masked_a.y1; row <= masked_a.y2; row++) {
      for (col = 0; col < map_useful_w; col++) {
        ...
        if (alpha_byte) {
          lv_opa_t px_opa =
              (*(((uint8_t *)px_color) + LV_IMG_PX_SIZE_ALPHA_BYTE - 1));
        ...
        if (recolor_opa != LV_OPA_TRANSP) {
          ...
          if (last_img_px.full != px_color->full) { /*Minor acceleration:
                                                       calculate only for new
          ...
          if (opa_result == LV_OPA_COVER)
            vdb_buf_tmp[col].full = px_color->full;
          else
            vdb_buf_tmp[col] =
