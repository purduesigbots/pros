...
void lv_vfill(const lv_area_t* cords_p, const lv_area_t* mask_p, lv_color_t color, lv_opa_t opa) {
	...
	/*Not opaque fill*/
		...
		else if (lv_area_get_height(&vdb_rel_a) > VFILL_HW_ACC_SIZE_LIMIT && lv_disp_is_mem_blend_supported()) {
 ...
 */
void lv_vletter(const lv_point_t* pos_p, const lv_area_t* mask_p, const lv_font_t* font_p, uint32_t letter,
	...
	if (pos_x + letter_w < mask_p->x1 || pos_x > mask_p->x2 || pos_y + letter_h < mask_p->y1 || pos_y > mask_p->y2)
		return;
	...
	vdb_buf_tmp += ((pos_y - vdb_p->area.y1) * vdb_width) + pos_x - vdb_p->area.x1;
	...
	for (row = row_start; row < row_end; row++) {
		...
		for (col = col_start; col < col_end; col++) {
			...
			if (letter_px != 0) {
				...
				if (disp->driver.vdb_wr) {
					disp->driver.vdb_wr((uint8_t*)vdb_p->buf, vdb_width, (col + pos_x) - vdb_p->area.x1,
 ...
 */
void lv_vmap(const lv_area_t* cords_p, const lv_area_t* mask_p, const uint8_t* map_p, lv_opa_t opa, bool chroma_key,
