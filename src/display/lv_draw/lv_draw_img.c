...
static lv_res_t lv_img_decoder_read_line(lv_coord_t x, lv_coord_t y, lv_coord_t len, uint8_t* buf) {
	...
	if (decoder_src_type == LV_IMG_SRC_FILE) {
		...
		if (decoder_header.cf == LV_IMG_CF_TRUE_COLOR || decoder_header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA ||
		...
		} else if (decoder_header.cf == LV_IMG_CF_INDEXED_1BIT || decoder_header.cf == LV_IMG_CF_INDEXED_2BIT ||
		           decoder_header.cf == LV_IMG_CF_INDEXED_4BIT || decoder_header.cf == LV_IMG_CF_INDEXED_8BIT) {
		...
		if (img_dsc->header.cf == LV_IMG_CF_ALPHA_1BIT || img_dsc->header.cf == LV_IMG_CF_ALPHA_2BIT ||
		    img_dsc->header.cf == LV_IMG_CF_ALPHA_4BIT || img_dsc->header.cf == LV_IMG_CF_ALPHA_8BIT) {
			lv_img_built_in_decoder_line_alpha(x, y, len, buf);
		} else if (img_dsc->header.cf == LV_IMG_CF_INDEXED_1BIT || img_dsc->header.cf == LV_IMG_CF_INDEXED_2BIT ||
		           img_dsc->header.cf == LV_IMG_CF_INDEXED_4BIT || img_dsc->header.cf == LV_IMG_CF_INDEXED_8BIT) {
			lv_img_built_in_decoder_line_indexed(x, y, len, buf);
		} else {
