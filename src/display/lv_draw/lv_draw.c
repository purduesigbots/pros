...
#define CIRCLE_AA_NON_LINEAR_OPA_THRESHOLD                                     \
  5 /*Circle segment greater then this value will be anti-aliased by a         \
       non-linear (cos) opacity mapping*/
...
#define SHADOW_OPA_EXTRA_PRECISION                                             \
  8 /*Calculate with 2^x bigger shadow opacity values to avoid rounding        \
       errors*/
 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_draw_rect_main_mid(const lv_area_t *coords,
                                  const lv_area_t *mask,
                                  const lv_style_t *style);
static void lv_draw_rect_main_corner(const lv_area_t *coords,
                                     const lv_area_t *mask,
                                     const lv_style_t *style);
static void lv_draw_rect_border_straight(const lv_area_t *coords,
                                         const lv_area_t *mask,
                                         const lv_style_t *style);
static void lv_draw_rect_border_corner(const lv_area_t *coords,
                                       const lv_area_t *mask,
                                       const lv_style_t *style);
...
static void lv_draw_shadow_full(const lv_area_t *coords, const lv_area_t *mask,
                                const lv_style_t *style);
static void lv_draw_shadow_bottom(const lv_area_t *coords,
                                  const lv_area_t *mask,
                                  const lv_style_t *style);
static void lv_draw_shadow_full_straight(const lv_area_t *coords,
                                         const lv_area_t *mask,
                                         const lv_style_t *style,
                                         const lv_opa_t *map);
#endif
static uint16_t lv_draw_cont_radius_corr(uint16_t r, lv_coord_t w,
                                         lv_coord_t h);
#if LV_ANTIALIAS != 0
static lv_opa_t antialias_get_opa(lv_coord_t seg, lv_coord_t px_id,
                                  lv_opa_t line_opa);
 ...
 **********************/
#if LV_VDB_SIZE != 0
static void (*px_fp)(lv_coord_t x, lv_coord_t y, const lv_area_t *mask,
                     lv_color_t color, lv_opa_t opa) = lv_vpx;
static void (*fill_fp)(const lv_area_t *coords, const lv_area_t *mask,
                       lv_color_t color, lv_opa_t opa) = lv_vfill;
static void (*letter_fp)(const lv_point_t *pos_p, const lv_area_t *mask,
                         const lv_font_t *font_p, uint32_t letter,
                         lv_color_t color, lv_opa_t opa) = lv_vletter;
#if USE_LV_IMG
 ...
 * static void (*px_fp)(lv_coord_t x, lv_coord_t y, const lv_area_t * mask,
 * lv_color_t color, lv_opa_t opa) = lv_rpx;
 */
static void (*fill_fp)(const lv_area_t *coords, const lv_area_t *mask,
                       lv_color_t color, lv_opa_t opa) = lv_rfill;
static void (*letter_fp)(const lv_point_t *pos_p, const lv_area_t *mask,
                         const lv_font_t *font_p, uint32_t letter,
                         lv_color_t color, lv_opa_t opa) = lv_rletter;
#if USE_LV_IMG
static void (*map_fp)(const lv_area_t *cords_p, const lv_area_t *mask_p,
                      const uint8_t *map_p, lv_opa_t opa, bool chroma_key,
                      bool alpha_byte, lv_color_t recolor,
...
static void (*px_fp)(lv_coord_t x, lv_coord_t y, const lv_area_t *mask,
                     lv_color_t color, lv_opa_t opa) = NULL;
static void (*fill_fp)(const lv_area_t *coords, const lv_area_t *mask,
                       lv_color_t color, lv_opa_t opa) = NULL;
static void (*letter_fp)(const lv_point_t *pos_p, const lv_area_t *mask,
                         const lv_font_t *font_p, uint32_t letter,
 ...
 */
void lv_draw_rect(const lv_area_t *coords, const lv_area_t *mask,
                  const lv_style_t *style) {
  ...
  if (style->body.border.width != 0 &&
      style->body.border.part != LV_BORDER_NONE) {
 ...
 */
void lv_draw_triangle(const lv_point_t *points, const lv_area_t *mask,
                      lv_color_t color) {
  lv_point_t tri[3];

  memcpy(tri, points, sizeof(tri));

  /*Sort the vertices according to their y coordinate (0: y max, 1: y mid, 2:y
   * min)*/
  if (tri[1].y < tri[0].y)
    point_swap(&tri[1], &tri[0]);
  if (tri[2].y < tri[1].y)
    point_swap(&tri[2], &tri[1]);
  if (tri[1].y < tri[0].y)
    point_swap(&tri[1], &tri[0]);
  ...
  while (1) {
    ...
    draw_area.x2--; /*Do not draw most right pixel because it will be drawn by
...
void lv_draw_label(const lv_area_t *coords, const lv_area_t *mask,
                   const lv_style_t *style, const char *txt, lv_txt_flag_t flag,
                   lv_point_t *offset) {
  ...
  uint32_t line_end =
      lv_txt_get_next_line(txt, font, style->text.letter_space, w, flag);
      ...
      if ((flag & LV_TXT_FLAG_RECOLOR) != 0) {
        if (letter == (uint32_t)LV_TXT_COLOR_CMD[0]) {
          if (cmd_state == CMD_STATE_WAIT) { /*Start char*/
            ...
            continue;
          } else if (cmd_state == CMD_STATE_PAR) { /*Other start char in
                                                      parameter escaped cmd.
                                                      char */
            cmd_state = CMD_STATE_WAIT;
  ...
  if (u8_p[0] >= 'A' && u8_p[0] <= 'Z') { /*It will be a path of a file*/
    ...
    if (res == LV_FS_RES_OK) {
      ...
      if (img_data.header.alpha_byte) { /*Correction with the alpha byte*/
        px_size++;
        if (img_data.header.format == LV_IMG_FORMAT_FILE_RAW_RGB888)
          px_size--; /*Stored in the 4 byte anyway*/
      }

      /* Move the file pointer to the start address according to mask*/
      uint32_t start_offset = sizeof(img_data.header);
      start_offset += (lv_area_get_width(coords)) * (mask_com.y1 - coords->y1) *
                      px_size;                              /*First row*/
      ...
      for (row = mask_com.y1; row <= mask_com.y2; row++) {
        ...
        map_fp(&line, &mask_com, (uint8_t *)buf, style->image.opa,
               img_data.header.chroma_keyed, img_data.header.alpha_byte,
      ...
      if (res != LV_FS_RES_OK) {
        lv_draw_rect(coords, mask, &lv_style_plain);
        lv_draw_label(coords, mask, &lv_style_plain, "No data",
                      LV_TXT_FLAG_NONE, NULL);
      }
    }
#endif
  } else {
    ...
    map_fp(coords, mask, img_var->pixel_map, style->image.opa,
           img_var->header.chroma_keyed, img_var->header.alpha_byte,
           style->image.color, style->image.intense);
  }
}
#endif
 ...
 */
void lv_draw_line(const lv_point_t *point1, const lv_point_t *point2,
                  const lv_area_t *mask, const lv_style_t *style) {
  ...
  static const uint8_t width_corr_array[] = {
      64, 64, 64, 64, 64, 64, 64, 64, 64, 65, 65, 65, 65, 65, 66, 66, 66,
      66, 66, 67, 67, 67, 68, 68, 68, 69, 69, 69, 70, 70, 71, 71, 72, 72,
      72, 73, 73, 74, 74, 75, 75, 76, 77, 77, 78, 78, 79, 79, 80, 81, 81,
      82, 82, 83, 84, 84, 85, 86, 86, 87, 88, 88, 89, 90, 91,
  };
  ...
  aa_invert =
      p1.y < p2.y ? false : true; /*Direction of opacity increase on the edges*/
  if (p1.x != p2.x && p1.y != p2.y)
    width--; /*Because of anti aliasing (no anti aliasing on hor. and ver.
                lines)*/
#endif
  ...
  while (1) {
    if (hor == true && last_y != act_point.y) {
      ...
      for (i = 0; i < seg_w; i++) {
        ...
        px_fp(aa_p1.x + i, aa_p1.y, mask, style->line.color,
              aa_invert ? aa_opa : style->line.opa - aa_opa);
        px_fp(aa_p2.x + i, aa_p2.y, mask, style->line.color,
              aa_invert ? style->line.opa - aa_opa : aa_opa);
      }
#endif
    }
    if (hor == false && last_x != act_point.x) {
      ...
      for (i = 0; i < seg_h; i++) {
        ...
        px_fp(aa_p1.x, aa_p1.y + i, mask, style->line.color,
              aa_invert ? aa_opa : style->line.opa - aa_opa);
        px_fp(aa_p2.x, aa_p2.y + i, mask, style->line.color,
              aa_invert ? style->line.opa - aa_opa : aa_opa);
      }
#endif
    }
    ...
    if (act_point.x == p2.x && act_point.y == p2.y)
      break;
    e2 = err;
    if (e2 > -dx) {
      err -= dy;
...
static void lv_draw_rect_main_corner(const lv_area_t *coords,
  ...
  while (lv_circ_cont(&cir)) {
    ...
    if (out_x_last != cir.x) {
      ...
      for (i = 0; i < seg_size; i++) {
        lv_opa_t aa_opa;
        if (seg_size >
            CIRCLE_AA_NON_LINEAR_OPA_THRESHOLD) { /*Use non-linear opa mapping
        ...
        px_fp(rb_origo.x + LV_CIRC_OCT2_X(aa_p) + i,
              rb_origo.y + LV_CIRC_OCT2_Y(aa_p) + 1, mask, aa_color_hor_bottom,
              aa_opa);
        px_fp(lb_origo.x + LV_CIRC_OCT3_X(aa_p) - i,
              lb_origo.y + LV_CIRC_OCT3_Y(aa_p) + 1, mask, aa_color_hor_bottom,
              aa_opa);
        px_fp(lt_origo.x + LV_CIRC_OCT6_X(aa_p) - i,
              lt_origo.y + LV_CIRC_OCT6_Y(aa_p) - 1, mask, aa_color_hor_top,
              aa_opa);
        px_fp(rt_origo.x + LV_CIRC_OCT7_X(aa_p) + i,
              rt_origo.y + LV_CIRC_OCT7_Y(aa_p) - 1, mask, aa_color_hor_top,
              aa_opa);

        mix =
            (uint32_t)((uint32_t)(radius - out_y_seg_start + i) * 255) / height;
        aa_color_ver = lv_color_mix(mcolor, gcolor, mix);
        px_fp(rb_origo.x + LV_CIRC_OCT1_X(aa_p) + 1,
              rb_origo.y + LV_CIRC_OCT1_Y(aa_p) + i, mask, aa_color_ver,
              aa_opa);
        px_fp(lb_origo.x + LV_CIRC_OCT4_X(aa_p) - 1,
              lb_origo.y + LV_CIRC_OCT4_Y(aa_p) + i, mask, aa_color_ver,
              aa_opa);

        aa_color_ver = lv_color_mix(gcolor, mcolor, mix);
        px_fp(lt_origo.x + LV_CIRC_OCT5_X(aa_p) - 1,
              lt_origo.y + LV_CIRC_OCT5_Y(aa_p) - i, mask, aa_color_ver,
              aa_opa);
        px_fp(rt_origo.x + LV_CIRC_OCT8_X(aa_p) + 1,
              rt_origo.y + LV_CIRC_OCT8_Y(aa_p) - i, mask, aa_color_ver,
              aa_opa);
      }
    ...
    if (edge_top_refr != 0) {
      if (mcolor.full == gcolor.full)
        ...
        mix = (uint32_t)((uint32_t)(coords->y2 - edge_top_area.y1) * 255) /
              height;
        act_color = lv_color_mix(mcolor, gcolor, mix);
      }
    ...
    if (mid_top_refr != 0) {
      if (mcolor.full == gcolor.full)
        ...
        mix =
            (uint32_t)((uint32_t)(coords->y2 - mid_top_area.y1) * 255) / height;
        act_color = lv_color_mix(mcolor, gcolor, mix);
      }
    ...
    if (mid_bot_refr != 0) {
      if (mcolor.full == gcolor.full)
        ...
        mix =
            (uint32_t)((uint32_t)(coords->y2 - mid_bot_area.y1) * 255) / height;
        act_color = lv_color_mix(mcolor, gcolor, mix);
      }
    ...
    if (edge_bot_refr != 0) {
      ...
      if (mcolor.full == gcolor.full)
        ...
        mix = (uint32_t)((uint32_t)(coords->y2 - edge_bot_area.y1) * 255) /
              height;
        act_color = lv_color_mix(mcolor, gcolor, mix);
      }
  ...
  if (edge_bot_area.y1 != mid_bot_area.y1) {
    ...
    if (mcolor.full == gcolor.full)
      ...
      mix =
          (uint32_t)((uint32_t)(coords->y2 - edge_bot_area.y1) * 255) / height;
      act_color = lv_color_mix(mcolor, gcolor, mix);
    }
    fill_fp(&edge_bot_area, mask, act_color, opa);
  }
  ...
  for (i = 0; i < seg_size; i++) {
    ...
    px_fp(rb_origo.x + LV_CIRC_OCT2_X(aa_p) + i,
          rb_origo.y + LV_CIRC_OCT2_Y(aa_p) + 1, mask, aa_color_hor_top,
          aa_opa);
    px_fp(lb_origo.x + LV_CIRC_OCT3_X(aa_p) - i,
          lb_origo.y + LV_CIRC_OCT3_Y(aa_p) + 1, mask, aa_color_hor_top,
          aa_opa);
    px_fp(lt_origo.x + LV_CIRC_OCT6_X(aa_p) - i,
          lt_origo.y + LV_CIRC_OCT6_Y(aa_p) - 1, mask, aa_color_hor_bottom,
          aa_opa);
    px_fp(rt_origo.x + LV_CIRC_OCT7_X(aa_p) + i,
          rt_origo.y + LV_CIRC_OCT7_Y(aa_p) - 1, mask, aa_color_hor_bottom,
          aa_opa);
  ...
  if (LV_MATH_ABS(aa_p.x - aa_p.y) == seg_size) {
    ...
    px_fp(rb_origo.x + LV_CIRC_OCT2_X(aa_p), rb_origo.y + LV_CIRC_OCT2_Y(aa_p),
          mask, aa_color_hor_bottom, aa_opa);
    px_fp(lb_origo.x + LV_CIRC_OCT4_X(aa_p), lb_origo.y + LV_CIRC_OCT4_Y(aa_p),
          mask, aa_color_hor_bottom, aa_opa);
    px_fp(lt_origo.x + LV_CIRC_OCT6_X(aa_p), lt_origo.y + LV_CIRC_OCT6_Y(aa_p),
          mask, aa_color_hor_top, aa_opa);
    px_fp(rt_origo.x + LV_CIRC_OCT8_X(aa_p), rt_origo.y + LV_CIRC_OCT8_Y(aa_p),
          mask, aa_color_hor_top, aa_opa);
  }
...
static void lv_draw_rect_border_corner(const lv_area_t *coords,
  ...
  while (cir_out.y <= cir_out.x) {
    ...
    if (out_x_last != cir_out.x) {
      ...
      for (i = 0; i < seg_size; i++) {
        ...
        if (seg_size >
          ...
          aa_opa = style->body.border.opa -
                   antialias_get_opa(seg_size, i, style->body.border.opa);
        }
      ...
      for (i = 0; i < seg_size; i++) {
        ...
        if (seg_size >
            CIRCLE_AA_NON_LINEAR_OPA_THRESHOLD) { /*Use non-linear opa mapping
                                                     on the first segment*/
          aa_opa = style->body.border.opa -
                   antialias_get_opa_circ(seg_size, i, style->body.border.opa);
        } else {
    ...
    if ((part & LV_BORDER_TOP) && (part & LV_BORDER_LEFT)) {
      if (lb_origo.y + LV_CIRC_OCT4_Y(cir_out) >
          lt_origo.y + LV_CIRC_OCT5_Y(cir_out)) {
    ...
    if ((part & LV_BORDER_TOP) && (part & LV_BORDER_RIGHT)) {
      ...
      /*Don't draw if the lines are common in the middle*/
      if (rb_origo.y + LV_CIRC_OCT1_Y(cir_out) >
          rt_origo.y + LV_CIRC_OCT8_Y(cir_out)) {
  ...
  lv_coord_t i;
  for (i = 0; i < seg_size; i++) {
    lv_opa_t aa_opa = style->body.border.opa -
                      antialias_get_opa(seg_size, i, style->body.border.opa);
    if ((part & LV_BORDER_BOTTOM) && (part & LV_BORDER_RIGHT)) {
      px_fp(rb_origo.x + LV_CIRC_OCT1_X(aa_p) + 1,
            rb_origo.y + LV_CIRC_OCT1_Y(aa_p) + i, mask,
  ...
  if (LV_MATH_ABS(aa_p.x - aa_p.y) == seg_size) {
    ...
    if ((part & LV_BORDER_BOTTOM) && (part & LV_BORDER_RIGHT)) {
      px_fp(rb_origo.x + LV_CIRC_OCT2_X(aa_p),
            rb_origo.y + LV_CIRC_OCT2_Y(aa_p), mask, style->body.border.color,
            aa_opa);
    }

    if ((part & LV_BORDER_BOTTOM) && (part & LV_BORDER_LEFT)) {
      px_fp(lb_origo.x + LV_CIRC_OCT4_X(aa_p),
            lb_origo.y + LV_CIRC_OCT4_Y(aa_p), mask, style->body.border.color,
            aa_opa);
    }

    if ((part & LV_BORDER_TOP) && (part & LV_BORDER_LEFT)) {
      px_fp(lt_origo.x + LV_CIRC_OCT6_X(aa_p),
            lt_origo.y + LV_CIRC_OCT6_Y(aa_p), mask, style->body.border.color,
            aa_opa);
    }

    if ((part & LV_BORDER_TOP) && (part & LV_BORDER_RIGHT)) {
      px_fp(rt_origo.x + LV_CIRC_OCT8_X(aa_p),
            rt_origo.y + LV_CIRC_OCT8_Y(aa_p), mask, style->body.border.color,
            aa_opa);
    }
  }
...
static void lv_draw_shadow(const lv_area_t *coords, const lv_area_t *mask,
  ...
  area_tmp.x1 += radius;
  area_tmp.x2 -= radius;
  if (lv_area_is_in(mask, &area_tmp) != false)
    return;
...
static void lv_draw_shadow_full(const lv_area_t *coords, const lv_area_t *mask,
  ...
  lv_coord_t curve_x[radius + swidth +
                     1]; /*Stores the 'x' coordinates of a quarter circle.*/
  ...
  for (line = 0; line < filter_width; line++) {
    line_1d_blur[line] =
        (uint32_t)((uint32_t)(filter_width - line) * (style->body.opa * 2)
  ...
  bool line_ready;
  for (line = 1; line <= radius + swidth;
       line++) { /*Check all rows and make the 1D blur to 2D*/
    line_ready = false;
    for (col = 1; col < radius + swidth + 10;
         col++) { /*Check all pixels in a 1D blur line (from the origo to last
      ...
      for (line_rel = -swidth; line_rel <= swidth; line_rel++) {
        ...
        int16_t col_rel;
        if (line + line_rel <
            0) { /*Below the radius, here is the blur of the edge */
        ...
        if (col_rel < -swidth) { /*Outside of the burred area. */
          if (line_rel == -swidth)
            line_ready = true; /*If no data even on the very first line then it
                                  wont't be anything else in this line*/
          ...
          px_opa_sum += line_1d_blur[swidth - col_rel]; /*On the 1D blur (+
                                                           swidth to align to
                                                           the center)*/
      }
    ...
    for (d = 1; d <= col; d++) {
      ...
      if (point_rt.x != point_lt.x) {
        px_fp(point_lt.x, point_lt.y, mask, style->body.shadow.color,
              line_2d_blur[d]);
      }

      if (point_rb.x != point_lb.x && point_lt.y != point_lb.y) {
        px_fp(point_lb.x, point_lb.y, mask, style->body.shadow.color,
              line_2d_blur[d]);
      }

      if (point_lt.y != point_lb.y) {
        px_fp(point_rb.x, point_rb.y, mask, style->body.shadow.color,
              line_2d_blur[d]);
      }

      px_fp(point_rt.x, point_rt.y, mask, style->body.shadow.color,
            line_2d_blur[d]);
...
static void lv_draw_shadow_bottom(const lv_area_t *coords,
  ...
  lv_coord_t
      curve_x[radius + 1]; /*Stores the 'x' coordinates of a quarter circle.*/
  lv_point_t circ;
  lv_coord_t circ_tmp;
  lv_circ_init(&circ, &circ_tmp, radius);
  while (lv_circ_cont(&circ)) {
  ...
  for (col = 0; col < swidth; col++) {
    line_1d_blur[col] =
        (uint32_t)((uint32_t)(swidth - col) * style->body.opa / 2) / (swidth);
  }
  ...
  for (col = 0; col <= radius; col++) {
    ...
    uint16_t d;
    for (d = 0; d < swidth; d++) {
      /*When stepping a pixel in y calculate the average with the pixel from the
       * prev. column to make a blur */
      if (diff == 0) {
        px_opa = line_1d_blur[d];
      } else {
        px_opa =
            (uint16_t)((uint16_t)line_1d_blur[d] + line_1d_blur[d - diff]) >> 1;
      }
...
static lv_opa_t antialias_get_opa(lv_coord_t seg, lv_coord_t px_id,
   ...
   * anti-aliased (was 2 in the example)
...
static lv_opa_t antialias_get_opa_circ(lv_coord_t seg, lv_coord_t px_id,
  ...
  if (seg == 0)
    ...
    uint8_t id =
        (uint32_t)((uint32_t)px_id * (sizeof(opa_map) - 1)) / (seg - 1);
