...
static void indev_pointer_proc(lv_indev_t* i, lv_indev_data_t* data) {
	...
	if (i->cursor != NULL && (i->proc.last_point.x != data->point.x || i->proc.last_point.y != data->point.y)) {
...
static void indev_keypad_proc(lv_indev_t* i, lv_indev_data_t* data) {
	...
	/*Key press happened*/
	if (data->state == LV_INDEV_STATE_PR && i->proc.last_state == LV_INDEV_STATE_REL) {
		...
		if (i->proc.long_pr_sent == 0 && lv_tick_elaps(i->proc.pr_timestamp) > LV_INDEV_LONG_PRESS_TIME) {
			...
			if (editable) {
				if (i->group->obj_ll.head != i->group->obj_ll.tail)
					lv_group_set_editing(i->group,
					                     lv_group_get_editing(i->group) ? false : true); /*Toggle edit mode on long press*/
				else if (focused)
					focused->signal_func(focused, LV_SIGNAL_LONG_PRESS, indev_act);
			}
	...
	/*Release happened*/
		...
		/*An object is being edited and the button is releases. Just send enter */
			...
			if (!i->proc.long_pr_sent || i->group->obj_ll.head == i->group->obj_ll.tail)
				lv_group_send_data(i->group,
				                   LV_GROUP_KEY_ENTER); /*Ignore long pressed enter release because it comes from mode switch*/
		}
...
static void indev_button_proc(lv_indev_t* i, lv_indev_data_t* data) {
	...
	if (i->proc.last_point.x == i->proc.act_point.x && i->proc.last_point.y == i->proc.act_point.y &&
