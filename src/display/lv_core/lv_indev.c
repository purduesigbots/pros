...
void lv_indev_enable(lv_hal_indev_type_t type, bool enable) {
  ...
  while (i) {
    if (i->driver.type == type)
      i->proc.disabled = enable == false ? 1 : 0;
    i = lv_indev_next(i);
  }
}
...
void lv_indev_set_cursor(lv_indev_t *indev, lv_obj_t *cur_obj) {
  ...
  lv_obj_set_pos(indev->cursor, indev->proc.act_point.x,
                 indev->proc.act_point.y);
}
 ...
 * @param indev pointer to an input device (NULL to get the overall smallest
 * inactivity)
...
uint32_t lv_indev_get_inactive_time(lv_indev_t *indev) {
  ...
  if (indev)
    return t = lv_tick_elaps(indev->last_activity_time);
...
static void indev_pointer_proc(lv_indev_t *i, lv_indev_data_t *data) {
  ...
  if (i->cursor != NULL && (i->proc.last_point.x != data->point.x ||
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data) {
  ...
  /*Pressing*/
           ...
           i->proc.last_state == LV_INDEV_STATE_PR) {
    if (data->key == LV_GROUP_KEY_ENTER && i->proc.long_pr_sent == 0 &&
  ...
  /*Release happened*/
           ...
           i->proc.last_state == LV_INDEV_STATE_PR) {
    /*The user might clear the key it was released. Always release the pressed
     * key*/
...
static void indev_proc_press(lv_indev_proc_t *proc) {
  ...
  if (proc->act_obj == NULL) {
    ...
    if (pr_obj == NULL)
      pr_obj = indev_search_obj(proc, lv_scr_act());
  }
  /*If there is last object but it is not dragged and not protected also
     search*/
  else if (proc->drag_in_prog == 0 &&
           lv_obj_is_protected(proc->act_obj, LV_PROTECT_PRESS_LOST) ==
               false) { /*Now act_obj != NULL*/
    pr_obj = indev_search_obj(proc, lv_layer_top());
    if (pr_obj == NULL)
      pr_obj = indev_search_obj(proc, lv_scr_act());
  }
  ...
  if (pr_obj != proc->act_obj) {
    ...
    if (proc->act_obj != NULL) {
      proc->act_obj->signal_func(proc->act_obj, LV_SIGNAL_PRESS_LOST,
                                 indev_act);
      if (proc->reset_query != 0)
        return;
    }
...
static void indev_drag(lv_indev_proc_t *state) {
  ...
  if (state->drag_range_out != 0) {
    ...
    if (state->vect.x != 0 || state->vect.y != 0) {
      /*Get the coordinates of the object and modify them*/
      lv_coord_t act_x = lv_obj_get_x(drag_obj);
      lv_coord_t act_y = lv_obj_get_y(drag_obj);
      uint16_t inv_buf_size = lv_refr_get_buf_size(); /*Get the number of
