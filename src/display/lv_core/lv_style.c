...
#define STYLE_ATTR_ANIM(attr, r)                                               \
  if (start->attr != end->attr)                                                \
  act->attr = VAL_PROP(start->attr, end->attr, r)
#endif
...
typedef struct {
  lv_style_t style_start; /*Save not only pointers because can be same as
                             'style_anim' then it will be modified too*/
...
#if USE_LV_ANIMATION
/**
 * Used by the style animations to set the values of a style according to start
 * and end style.
...
static void style_animator(lv_style_anim_dsc_t *dsc, int32_t val) {
  ...
  act->body.main_color =
      lv_color_mix(end->body.main_color, start->body.main_color, opa);
  act->body.grad_color =
      lv_color_mix(end->body.grad_color, start->body.grad_color, opa);
  act->body.border.color =
      lv_color_mix(end->body.border.color, start->body.border.color, opa);
  act->body.shadow.color =
      lv_color_mix(end->body.shadow.color, start->body.shadow.color, opa);
