...
static void lv_refr_join_area(void) {
	...
	for (join_in = 0; join_in < inv_buf_p; join_in++) {
		...
		for (join_from = 0; join_from < inv_buf_p; join_from++) {
			...
			if (lv_area_is_on(&inv_buf[join_in].area, &inv_buf[join_from].area) == false) {
				continue;
			}

			lv_area_join(&joined_area, &inv_buf[join_in].area, &inv_buf[join_from].area);
...
static lv_obj_t* lv_refr_get_top_obj(const lv_area_t* area_p, lv_obj_t* obj) {
	...
	if (lv_area_is_in(area_p, &obj->coords) && obj->hidden == 0) {
		...
		if (found_p == NULL) {
			...
			if (style->body.opa == LV_OPA_COVER && obj->design_func(obj, area_p, LV_DESIGN_COVER_CHK) != false &&
