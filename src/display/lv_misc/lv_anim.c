...
int32_t lv_anim_path_linear(const lv_anim_t *a) {
  ...
  if (a->time == a->act_time)
    step = LV_ANIM_RESOLUTION; /*Use the last value id the time fully elapsed*/
  else
    step = (a->act_time * LV_ANIM_RESOLUTION) / a->time;

  /* Get the new value which will be proportional to the current element of
   * 'path_p'
  ...
  while (a != NULL) {
    ...
    if (a->act_time >= 0) {
      ...
      if (a->act_time >= a->time) {
        ...
        if (invalid != false) {
          a_next = lv_ll_get_head(&anim_ll); /*a_next might be invalid if
                                                animation delete occurred*/
        }
      }
    }
