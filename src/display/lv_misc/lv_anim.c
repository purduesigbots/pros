...
static void anim_task(void* param) {
	...
	while (a != NULL) {
		/*It can be set by `lv_anim_del()` typically in `end_cb`. If set then an animation delete happened in
		 * `anim_ready_handler` which could make this linked list reading corrupt because the list is changed meanwhile
		 */
