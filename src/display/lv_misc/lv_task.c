...
LV_ATTRIBUTE_TASK_HANDLER void lv_task_handler(void) {
	...
	bool expected = false;
	bool toSet = true;
	bool alreadyTaken = __atomic_compare_exchange(&task_handler_mutex, &expected, &toSet, /* weak*/ false,
	...
	do {
		...
		while (LV_GC_ROOT(_lv_task_act)) {
			...
			if (LV_GC_ROOT(_lv_task_act) == task_interrupter) {
				task_interrupter = NULL; /*From this point only task after the interrupter comes, so the interrupter is not
				                            interesting anymore*/
