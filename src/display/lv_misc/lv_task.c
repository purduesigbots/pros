    ...
    LL_READ(lv_task_ll, tmp) {
      ...
      /*Tasks with higher priority then the interrupted shall be run in every
        ...
        if (tmp->prio > task_interruper->prio) {
          if (lv_task_exec(tmp)) {
            task_interruper =
                tmp; /*Check all tasks again from the highest priority */
            end_flag = false;
            break;
          }
        }
...
void lv_task_set_prio(lv_task_t *lv_task_p, lv_task_prio_t prio) {
  /*It's easier to create a new task with the new priority rather then modify
   * the linked list*/
  lv_task_t *new_task = lv_task_create(lv_task_p->task, lv_task_p->period, prio,
                                       lv_task_p->param);
