...
lv_obj_t *lv_img_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_img);
...
void lv_img_set_src(lv_obj_t *img, const void *src_img) {
  ...
  if (src_type == LV_IMG_SRC_FILE)
    src_type = LV_IMG_SRC_UNKNOWN;
#endif

  if (src_type == LV_IMG_SRC_UNKNOWN) {
    if (ext->src_type == LV_IMG_SRC_SYMBOL ||
        ext->src_type == LV_IMG_SRC_FILE) {
      lv_mem_free(ext->src);
    }
    ext->src = NULL;
    ext->src_type = LV_IMG_SRC_UNKNOWN;
  ...
  if (src_type == LV_IMG_SRC_VARIABLE) {
    ...
    ext->w = size.x;
    ext->h = size.y;
    ext->chroma_keyed = 1; /*Symbols always have transparent parts, Important
                              because of cover check in the design function*/
...
lv_img_src_t lv_img_get_src_type(const void *src) {
  ...
  if (u8_p[0] >= 'A' && u8_p[0] <= 'Z')
    return LV_IMG_SRC_FILE; /*It's a driver letter*/
  else if (((u8_p[0] & 0xFC) >> 2) == LV_IMG_FORMAT_INTERNAL_RAW)
    return LV_IMG_SRC_VARIABLE; /*Mask the file format part og of lv_img_t
                                   header. IT should be 0 which means C array */
  else if (u8_p[0] >= ' ')
    return LV_IMG_SRC_SYMBOL; /*Other printable characters are considered
                                 symbols*/
...
static bool lv_img_design(lv_obj_t *img, const lv_area_t *mask,
  ...
  if (mode == LV_DESIGN_COVER_CHK) {
    bool cover = false;
    if (ext->src_type == LV_IMG_SRC_UNKNOWN ||
        ext->src_type == LV_IMG_SRC_SYMBOL)
      return false;

    if (ext->chroma_keyed == 0 && ext->alpha_byte == 0)
    ...
    if (ext->src_type == LV_IMG_SRC_FILE ||
        ext->src_type == LV_IMG_SRC_VARIABLE) {
      lv_area_t cords_tmp;
      cords_tmp.y1 = coords.y1;
      cords_tmp.y2 = coords.y1 + ext->h - 1;

      for (; cords_tmp.y1 < coords.y2;
           cords_tmp.y1 += ext->h, cords_tmp.y2 += ext->h) {
        cords_tmp.x1 = coords.x1;
        cords_tmp.x2 = coords.x1 + ext->w - 1;
        for (; cords_tmp.x1 < coords.x2;
             cords_tmp.x1 += ext->w, cords_tmp.x2 += ext->w) {
