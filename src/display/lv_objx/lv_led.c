 ...
 * @param copy pointer to a led object, if not NULL then the new object will be
 * copied from it
...
lv_obj_t *lv_led_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_led);
  if (ancestor_design_f == NULL)
    ancestor_design_f = lv_obj_get_design_func(new_led);
 ...
 * @param led pointer to a LED object
 */
void lv_led_on(lv_obj_t *led) { lv_led_set_bright(led, LV_LED_BRIGHT_ON); }

/**
 * Light off a LED
 * @param led pointer to a LED object
 */
void lv_led_off(lv_obj_t *led) { lv_led_set_bright(led, LV_LED_BRIGHT_OFF); }
...
static bool lv_led_design(lv_obj_t *led, const lv_area_t *mask,
  ...
  if (mode == LV_DESIGN_COVER_CHK) {
     ...
     * style
     * and to the real object style. It is important because of style change
     * tricks below*/
    ...
    /*Set the current swidth according to brightness proportionally between
     * LV_LED_BRIGHT_OFF and LV_LED_BRIGHT_ON*/
