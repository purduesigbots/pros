...
lv_obj_t *lv_page_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_page);
  if (ancestor_design == NULL)
    ancestor_design = lv_obj_get_design_func(new_page);

  /*Allocate the object type specific extended data*/
  lv_page_ext_t *ext =
      lv_obj_allocate_ext_attr(new_page, sizeof(lv_page_ext_t));
  lv_mem_assert(ext);
  ext->scrl = NULL;
  ext->pr_action = NULL;
  ext->rel_action = NULL;
  ...
  if (copy == NULL) {
    ...
    lv_page_set_style(new_page, LV_PAGE_STYLE_BG,
                      lv_page_get_style(copy, LV_PAGE_STYLE_BG));
    ...
    lv_page_set_style(new_page, LV_PAGE_STYLE_SB,
                      lv_page_get_style(copy, LV_PAGE_STYLE_SB));
 ...
 *  */
void lv_page_set_style(lv_obj_t *page, lv_page_style_t type,
                       lv_style_t *style) {
  lv_page_ext_t *ext = lv_obj_get_ext_attr(page);
...
void lv_page_focus(lv_obj_t *page, lv_obj_t *obj, uint16_t anim_time) {
  ...
  if ((obj_h <= page_h && top_err > 0) ||
    ...
    scrlable_y =
        -(obj_y - style_scrl->body.padding.ver - style->body.padding.ver);
...
static bool lv_scrl_design(lv_obj_t *scrl, const lv_area_t *mask,
  ...
  if (mode == LV_DESIGN_COVER_CHK) {
    ...
    if (style_page->body.empty != 0 ||
        style_page->body.opa == LV_OPA_TRANSP) { /*Background is visible?*/
      if (lv_group_get_focused(g) == page) {
        lv_style_t *style_mod;
        style_mod = lv_group_mod_style(g, style_ori);
        scrl->style_p =
            style_mod; /*Temporally change the style to the activated */
      }
    }
#endif
  ...
  if (sign == LV_SIGNAL_CHILD_CHG) { /*Automatically move children to the
    ...
    while (child != NULL) {
      if (lv_obj_is_protected(child, LV_PROTECT_PARENT) == false) {
        lv_obj_t *tmp = child;
        child = lv_obj_get_child(page,
                                 child); /*Get the next child before move this*/
    ...
    if (lv_cont_get_hor_fit(ext->scrl) == false) {
      lv_obj_set_width(ext->scrl,
                       lv_obj_get_width(page) - 2 * style->body.padding.hor);
    } else {
    ...
    /*Refresh the ext. size because the scrollbars might be positioned out of
     * the page*/
    lv_obj_refresh_ext_size(page);
  } else if (sign == LV_SIGNAL_CORD_CHG) {
    /*Refresh the scrollbar and notify the scrl if the size is changed*/
    if (ext->scrl != NULL &&
        (lv_obj_get_width(page) != lv_area_get_width(param) ||
    ...
    if (page->ext_size < (-ext->sb.style->body.padding.hor))
      page->ext_size = -ext->sb.style->body.padding.hor;
    if (page->ext_size < (-ext->sb.style->body.padding.ver))
      page->ext_size = -ext->sb.style->body.padding.ver;
...
static lv_res_t lv_page_scrollable_signal(lv_obj_t *scrl, lv_signal_t sign,
  ...
  if (sign == LV_SIGNAL_CORD_CHG) {
    ...
    if (lv_cont_get_hor_fit(scrl) == false) {
      lv_obj_set_width(
          scrl, lv_obj_get_width(page) - 2 * page_style->body.padding.hor);
    }
    ...
    if (lv_area_get_width(&scrl_cords) + 2 * hpad <
        lv_area_get_width(&page_cords)) {
      ...
      if (scrl_cords.x2 < page_cords.x2 - hpad) {
        new_x = lv_area_get_width(&page_cords) -
                lv_area_get_width(&scrl_cords) - hpad; /* Right align */
        refr_x = true;
      }
      if (scrl_cords.x1 > page_cords.x1 + hpad) {
    ...
    if (lv_area_get_height(&scrl_cords) + 2 * vpad <
        lv_area_get_height(&page_cords)) {
      ...
      if (scrl_cords.y2 < page_cords.y2 - vpad) {
        new_y = lv_area_get_height(&page_cords) -
                lv_area_get_height(&scrl_cords) - vpad; /* Bottom align */
        refr_y = true;
      }
      if (scrl_cords.y1 > page_cords.y1 + vpad) {
...
static void lv_page_sb_refresh(lv_obj_t *page) {
  ...
  lv_coord_t sb_hor_pad =
      LV_MATH_MAX(ext->sb.style->body.padding.inner, style->body.padding.hor);
  lv_coord_t sb_ver_pad =
      LV_MATH_MAX(ext->sb.style->body.padding.inner, style->body.padding.ver);

  if (ext->sb.mode == LV_SB_MODE_OFF)
    return;
