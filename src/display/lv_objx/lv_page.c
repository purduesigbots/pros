...
/**
 * Enable the scroll propagation feature. If enabled then the page will move its parent if there is no more space to
 * scroll.
...
static bool lv_scrl_design(lv_obj_t* scrl, const lv_area_t* mask, lv_design_mode_t mode) {
	if (mode == LV_DESIGN_COVER_CHK) {
		...
		if ((style_page->body.empty || style_page->body.opa == LV_OPA_TRANSP) &&
	...
	if (sign == LV_SIGNAL_CORD_CHG) {
		...
		if (lv_area_get_width(&scrl_coords) + 2 * hpad <= lv_area_get_width(&page_coords)) {
			...
			if (page_ext->scroll_prop_ip) {
				if (drag_vect.x == diff_x) { /*`scrl` is bouncing: drag pos. it somewhere and here it is reverted. Handle only
					                              the pos. because of drag*/
			...
			/*The edges of the scrollable can not be in the page (minus hpad) */
				...
				refr_x = true;
				if (page_ext->edge_flash.enabled && page_ext->edge_flash.left_ip == 0 && page_ext->edge_flash.right_ip == 0 &&
		...
		if (lv_area_get_height(&scrl_coords) + 2 * vpad <= lv_area_get_height(&page_coords)) {
			...
			/*If the scroll propagation is in progress revert the original coordinates (don't let the page scroll)*/
			if (page_ext->scroll_prop_ip) {
				if (drag_vect.y == diff_y) { /*`scrl` is bouncing: drag pos. it somewhere and here it is reverted. Handle only
					                              the pos. because of drag*/
...
static void lv_page_sb_refresh(lv_obj_t* page) {
	...
	if (scrl_w <= obj_w - 2 * hpad) { /*Full sized scroll bar*/
		...
		lv_area_set_pos(
		    &ext->sb.hor_area,
		    sb_hor_pad + (-(lv_obj_get_x(scrl) - hpad) * (obj_w - size_tmp - 2 * sb_hor_pad)) / (scrl_w + 2 * hpad - obj_w),
		    obj_h - ext->sb.style->body.padding.inner - ext->sb.style->body.padding.ver);
	...
	if (scrl_h <= obj_h - 2 * vpad) { /*Full sized scroll bar*/
		...
		lv_area_set_pos(&ext->sb.ver_area, obj_w - ext->sb.style->body.padding.inner - ext->sb.style->body.padding.hor,
		                sb_ver_pad + (-(lv_obj_get_y(scrl) - vpad) * (obj_h - size_tmp - 2 * sb_ver_pad)) /
		                                 (scrl_h + 2 * vpad - obj_h));
