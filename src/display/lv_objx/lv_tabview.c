...
lv_obj_t* lv_tabview_add_tab(lv_obj_t* tabview, const char* name) {
	...
	lv_coord_t indic_width = (lv_obj_get_width(tabview) - style_tabs->body.padding.inner * (ext->tab_cnt - 1) -
	                          2 * style_tabs->body.padding.hor) /
	                         ext->tab_cnt;
	lv_obj_set_width(ext->indic, indic_width);
	lv_obj_set_x(ext->indic, indic_width * ext->tab_cur + style_tabs->body.padding.inner * ext->tab_cur +
...
static void tabpage_pressing_handler(lv_obj_t* tabview, lv_obj_t* tabpage) {
	...
	if (ext->drag_hor) {
		...
		lv_coord_t p = ((tabpage->coords.x1 - tabview->coords.x1) * (indic_width + tabs_style->body.padding.inner)) /
		               lv_obj_get_width(tabview);

		lv_obj_set_x(ext->indic, indic_width * ext->tab_cur + tabs_style->body.padding.inner * ext->tab_cur +
...
static void tabview_realign(lv_obj_t* tabview) {
	...
	if (ext->btns_hide) {
		...
		/*Set the tabs height*/
		lv_coord_t btns_height = lv_font_get_height(style_btn_rel->text.font) + 2 * style_btn_rel->body.padding.ver +
