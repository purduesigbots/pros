...
static lv_res_t lv_roller_scrl_signal(lv_obj_t *roller_scrl, lv_signal_t sign,
                                      void *param);
static lv_res_t lv_roller_signal(lv_obj_t *roller, lv_signal_t sign,
                                 void *param);
...
lv_obj_t *lv_roller_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_scrl_signal == NULL)
    ancestor_scrl_signal = lv_obj_get_signal_func(lv_page_get_scrl(new_roller));
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_roller);

  /*Allocate the roller type specific extended data*/
  lv_roller_ext_t *ext =
      lv_obj_allocate_ext_attr(new_roller, sizeof(lv_roller_ext_t));
  lv_mem_assert(ext);
  ...
  if (copy == NULL) {
    ...
    lv_page_set_rel_action(new_roller,
                           NULL); /*Roller don't uses it (like ddlist)*/
    lv_page_set_scrl_fit(new_roller, true,
                         false); /*Height is specified directly*/
  ...
  /*Copy an existing roller*/
    ...
    lv_obj_refresh_style(
        new_roller); /*Refresh the style with new signal function*/
  }
...
void lv_roller_set_visible_row_count(lv_obj_t *roller, uint8_t row_cnt) {
  ...
  lv_ddlist_set_fix_height(roller,
                           lv_font_get_height(style_label->text.font) *
                                   row_cnt +
                               style_label->text.line_space * (row_cnt));
}
/**
  ...
  /*Post draw when the children are drawn*/
    ...
    if (area_ok) {
      ...
      lv_draw_label(&ext->ddlist.label->coords, &mask_sel, &new_style,
                    lv_label_get_text(ext->ddlist.label), LV_TXT_FLAG_CENTER,
                    NULL);
    }
  }
...
static lv_res_t lv_roller_signal(lv_obj_t *roller, lv_signal_t sign,
   ...
   * differently*/
  if (sign != LV_SIGNAL_CONTROLL && sign != LV_SIGNAL_FOCUS &&
  ...
  if (sign == LV_SIGNAL_STYLE_CHG) {
    lv_obj_set_height(lv_page_get_scrl(roller),
                      lv_obj_get_height(ext->ddlist.label) +
                          lv_obj_get_height(roller));
...
static lv_res_t lv_roller_scrl_signal(lv_obj_t *roller_scrl, lv_signal_t sign,
  ...
  if (sign == LV_SIGNAL_DRAG_END) {
    ...
    if (id < 0)
      id = 0;
    if (id >= ext->ddlist.option_cnt)
      id = ext->ddlist.option_cnt - 1;
...
static void draw_bg(lv_obj_t *roller, const lv_area_t *mask) {
  ...
  half_roller.x1 -=
      roller->ext_size; /*Add ext size too (e.g. because of shadow draw) */
  ...
  if ((h & 0x1) == 0)
    half_roller.y1++; /*With even height the pixels in the middle would be drawn
                         twice*/
