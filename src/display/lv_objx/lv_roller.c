...
void lv_roller_set_visible_row_count(lv_obj_t* roller, uint8_t row_cnt) {
	...
	lv_ddlist_set_fix_height(
	    roller, lv_font_get_height(style_label->text.font) * row_cnt + style_label->text.line_space * n_line_space);
}
...
static bool lv_roller_design(lv_obj_t* roller, const lv_area_t* mask, lv_design_mode_t mode) {
	...
	/*Post draw when the children are drawn*/
		...
		if (area_ok) {
			...
			lv_draw_label(&ext->ddlist.label->coords, &mask_sel, &new_style, opa_scale, lv_label_get_text(ext->ddlist.label),
			              txt_align, NULL);
		}
	}
	...
	if (sign == LV_SIGNAL_STYLE_CHG) {
		...
		if (indev_type == LV_INDEV_TYPE_ENCODER) {
			...
			ext->ddlist.sel_opt_id_ori =
			    ext->ddlist.sel_opt_id; /*Save the current value. Used to revert this state if ENER wont't be pressed*/
		}
#endif
...
static lv_res_t lv_roller_scrl_signal(lv_obj_t* roller_scrl, lv_signal_t sign, void* param) {
	...
	if (ext->ddlist.label == NULL)
		return LV_RES_INV; /*On delete the ddlist signal deletes the label so nothing left to do here*/
