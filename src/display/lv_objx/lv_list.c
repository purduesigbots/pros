...
#define LV_LIST_FOCUS_TIME                                                     \
  100 /*Animation time of focusing to the a list element [ms] (0: no           \
         animation)  */
#endif
#else
 ...
 * @param copy pointer to a list object, if not NULL then the new object will be
 * copied from it
...
lv_obj_t *lv_list_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_list);

  lv_list_ext_t *ext =
      lv_obj_allocate_ext_attr(new_list, sizeof(lv_list_ext_t));
  lv_mem_assert(ext);
  ...
  if (copy == NULL) {
    ...
    while (copy_btn) {
      ...
      if (copy_img)
        lv_img_create(new_btn, copy_img);
#endif
...
lv_obj_t *lv_list_add(lv_obj_t *list, const void *img_src, const char *txt,
  ...
  if (img_src) {
    ...
    if (img_signal == NULL)
      img_signal = lv_obj_get_signal_func(img);
  }
#endif
  if (txt != NULL) {
    ...
    if (label_signal == NULL)
      label_signal = lv_obj_get_signal_func(label);
  }
 ...
 */
void lv_list_set_style(lv_obj_t *list, lv_list_style_t type,
                       lv_style_t *style) {
  lv_list_ext_t *ext = lv_obj_get_ext_attr(list);
...
static lv_res_t lv_list_signal(lv_obj_t *list, lv_signal_t sign, void *param) {
  ...
  if (sign == LV_SIGNAL_CORD_CHG) {
    ...
    /*Because of the possible change of horizontal and vertical padding refresh
     * buttons width */
    refr_btn_width(list);
...
static void refr_btn_width(lv_obj_t *list) {
  ...
  lv_coord_t w = lv_obj_get_width(list);
  lv_coord_t btn_w =
      w - (style->body.padding.hor + style_scrl->body.padding.hor) * 2;
