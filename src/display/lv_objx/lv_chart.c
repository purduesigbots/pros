...
lv_obj_t *lv_chart_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  lv_chart_ext_t *ext =
      lv_obj_allocate_ext_attr(new_chart, sizeof(lv_chart_ext_t));
  lv_mem_assert(ext);
  ...
  if (ancestor_design_f == NULL)
    ancestor_design_f = lv_obj_get_design_func(new_chart);
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_chart);
 ...
 * @param chart pointer to chart object
 */
void lv_chart_refresh(lv_obj_t *chart) { lv_obj_invalidate(chart); }
 ...
 */
static lv_res_t lv_chart_signal(lv_obj_t *chart, lv_signal_t sign,
                                void *param) {
  lv_res_t res;
  ...
  if (sign == LV_SIGNAL_CLEANUP) {
    lv_coord_t **datal;
    lv_chart_ext_t *ext = lv_obj_get_ext_attr(chart);
    LL_READ(ext->series_ll, datal) { lv_mem_free(*datal); }
  ...
  if (ext->hdiv_cnt != 0) {
    ...
    for (div_i = div_i_start; div_i <= div_i_end; div_i++) {
      ...
      if (div_i == div_i_start)
        p1.y += (style->line.width >> 1) +
                1; /*The first line might not be visible*/
      if (div_i == div_i_end)
        p1.y -=
            (style->line.width >> 1) + 1; /*The last line might not be visible*/
...
static void lv_chart_draw_points(lv_obj_t *chart, const lv_area_t *mask) {
  ...
  LL_READ_BACK(ext->series_ll, ser) {
    ...
    style_point.body.grad_color =
        lv_color_mix(LV_COLOR_BLACK, ser->color, ext->series.dark);
...
static void lv_chart_draw_cols(lv_obj_t *chart, const lv_area_t *mask) {
  ...
  lv_chart_series_t *ser;
  lv_style_t rects;
  lv_coord_t col_w = w / ((ext->series.num + 1) *
                          ext->point_cnt); /* Suppose + 1 series as separator*/
