...
#include "display/lv_objx/lv_templ.h"
 ...
 *  STATIC PROTOTYPES
 **********************/
static bool lv_templ_design(lv_obj_t *templ, const area_t *mask,
...
lv_obj_t *lv_templ_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_templ);
  if (ancestor_design == NULL)
    ancestor_design = lv_obj_get_design_func(new_templ);
 ...
 * object is deleted
 */
static lv_res_t lv_templ_signal(lv_obj_t *templ, lv_signal_t sign,
                                void *param) {
  lv_res_t res;
