...
#define LV_SLIDER_SIZE_MIN                                                     \
  4 /*hor. pad and ver. pad cannot make the bar or indicator smaller then this \
       [px]*/
...
static lv_res_t lv_slider_signal(lv_obj_t *slider, lv_signal_t sign,
                                 void *param);
 ...
 * @param copy pointer to a slider object, if not NULL then the new object will
 * be copied from it
...
lv_obj_t *lv_slider_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_design_f == NULL)
    ancestor_design_f = lv_obj_get_design_func(new_slider);
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_slider);

  /*Allocate the slider type specific extended data*/
  lv_slider_ext_t *ext =
      lv_obj_allocate_ext_attr(new_slider, sizeof(lv_slider_ext_t));
  lv_mem_assert(ext);
...
static bool lv_slider_design(lv_obj_t *slider, const lv_area_t *mask,
  ...
  /*Draw the object*/
    ...
    lv_style_t *style_indic =
        lv_slider_get_style(slider, LV_SLIDER_STYLE_INDIC);
    ...
    area_bg.x1 +=
        slider_w < slider_h ? pad_hor_bg : 0; /*Pad only for vertical slider*/
    area_bg.x2 -=
        slider_w < slider_h ? pad_hor_bg : 0; /*Pad only for vertical slider*/
    area_bg.y1 +=
        slider_w > slider_h ? pad_ver_bg : 0; /*Pad only for horizontal slider*/
    area_bg.y2 -=
        slider_w > slider_h ? pad_ver_bg : 0; /*Pad only for horizontal slider*/
    ...
    if (ext->drag_value != LV_SLIDER_NOT_PRESSED)
      cur_value = ext->drag_value;

    if (slider_w >= slider_h) {
      area_indic.x2 = (int32_t)((int32_t)lv_area_get_width(&area_indic) *
                                (cur_value - min_value)) /
                      (max_value - min_value);
      area_indic.x2 += area_indic.x1;

    } else {
      area_indic.y1 = (int32_t)((int32_t)lv_area_get_height(&area_indic) *
                                (cur_value - min_value)) /
                      (max_value - min_value);
...
static lv_res_t lv_slider_signal(lv_obj_t *slider, lv_signal_t sign,
  ...
  if (sign == LV_SIGNAL_PRESSED) {
    ...
    if (w > h) {
      lv_coord_t knob_w = h;
      p.x -= slider->coords.x1 + h / 2; /*Modify the point to shift with half
                                           knob (important on the start and
                                           end)*/
      tmp = (int32_t)((int32_t)p.x *
                      (ext->bar.max_value - ext->bar.min_value + 1)) /
            (w - knob_w);
      tmp += ext->bar.min_value;
    } else {
      lv_coord_t knob_h = w;
      p.y -= slider->coords.y1 + w / 2; /*Modify the point to shift with half
                                           knob (important on the start and
                                           end)*/
      tmp = (int32_t)((int32_t)p.y *
                      (ext->bar.max_value - ext->bar.min_value + 1)) /
            (h - knob_h);
      tmp = ext->bar.max_value -
            tmp; /*Invert the value: smaller value means higher y*/
    }
     ...
     * During the drawing method the ext. size is used by the knob so refresh
     * the ext. size.*/
    ...
    if (ext->knob_in == 0) {
      ...
      if (slider->ext_size < x)
        slider->ext_size = x;
    } else {
      lv_coord_t pad =
          LV_MATH_MIN(style->body.padding.hor, style->body.padding.ver);
      if (pad < 0)
        pad = -pad;
      if (slider->ext_size < pad)
        slider->ext_size = pad;

      if (slider->ext_size < shadow_w)
        slider->ext_size = shadow_w;
    }
