...
lv_obj_t *lv_label_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_label);
 ...
 */
void lv_label_set_array_text(lv_obj_t *label, const char *array,
                             uint16_t size) {
  lv_obj_invalidate(label);
...
void lv_label_set_long_mode(lv_obj_t *label, lv_label_long_mode_t long_mode) {
  ...
  if (long_mode == LV_LABEL_LONG_ROLL)
    ext->expand = 1;
  else
    ext->expand = 0;
...
void lv_label_set_align(lv_obj_t *label, lv_label_align_t align) {
  ...
  lv_obj_invalidate(label); /*Enough to invalidate because alignment is only
                               drawing related (lv_refr_label_text() not
                               required)*/
}
...
void lv_label_set_recolor(lv_obj_t *label, bool recolor_en) {
  ...
  lv_label_refr_text(
      label); /*Refresh the text because the potential colo codes in text needs
                 to be hided or revealed*/
}
...
void lv_label_get_letter_pos(lv_obj_t *label, uint16_t index, lv_point_t *pos) {
  ...
  while (txt[new_line_start] != '\0') {
    new_line_start += lv_txt_get_next_line(
        &txt[line_start], font, style->text.letter_space, max_w, flag);
...
uint16_t lv_label_get_letter_on(lv_obj_t *label, lv_point_t *pos) {
  ...
  while (i < new_line_start - 1) {
    letter = lv_txt_utf8_next(
        txt, &i); /*Be careful 'i' already points to the next character*/
    /*Handle the recolor command*/
    if ((flag & LV_TXT_FLAG_RECOLOR) != 0) {
      if (lv_txt_is_cmd(&cmd_state, txt[i]) != false) {
        continue; /*Skip the letter is it is part of a command*/
 ...
 * object is deleted
 */
static lv_res_t lv_label_signal(lv_obj_t *label, lv_signal_t sign,
                                void *param) {
  lv_res_t res;
...
static void lv_label_refr_text(lv_obj_t *label) {
  ...
  /*In roll mode keep the size but start offset animations*/
    ...
    if (size.x > lv_obj_get_width(label)) {
      anim.end =
          lv_obj_get_width(label) - size.x - lv_font_get_width(font, ' ');
    ...
    if (size.y <=
        lv_obj_get_height(
            label)) { /*No dots are required, the text is short enough*/
      ext->dot_end = LV_LABEL_DOT_END_INV;
    } else if (lv_txt_get_length(ext->text) <=
               LV_LABEL_DOT_NUM) { /*Don't turn to dots all the characters*/
      ext->dot_end = LV_LABEL_DOT_END_INV;
    } else {
      lv_point_t p;
      p.x = lv_obj_get_width(label) -
            (lv_font_get_width(style->text.font, '.') +
             style->text.letter_space) *
