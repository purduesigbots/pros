...
void lv_label_set_long_mode(lv_obj_t* label, lv_label_long_mode_t long_mode) {
	...
	if (long_mode == LV_LABEL_LONG_ROLL || long_mode == LV_LABEL_LONG_CROP)
		ext->expand = 1;
	else
		ext->expand = 0;
...
void lv_label_set_align(lv_obj_t* label, lv_label_align_t align) {
	...
	lv_obj_invalidate(
	    label); /*Enough to invalidate because alignment is only drawing related (lv_refr_label_text() not required)*/
}
 ...
 * @param label pointer to a label object
 * @param index index of the letter [0 ... text length]. Expressed in character index, not byte index (different in
 * UTF-8)
...
static void lv_label_refr_text(lv_obj_t* label) {
	...
	if (ext->long_mode == LV_LABEL_LONG_EXPAND || ext->long_mode == LV_LABEL_LONG_SCROLL) {
		...
		if (ext->long_mode == LV_LABEL_LONG_SCROLL) {
			...
			anim.playback_pause =
			    (((lv_font_get_width(style->text.font, ' ') + style->text.letter_space) * 1000) / ext->anim_speed) *
	...
	/*In roll mode keep the size but start offset animations*/
		...
		if (size.y <= lv_obj_get_height(label)) { /*No dots are required, the text is short enough*/
			...
			lv_point_t p;
			p.x = lv_obj_get_width(label) - (lv_font_get_width(style->text.font, '.') + style->text.letter_space) *
