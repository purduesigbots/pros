...
static void lv_cont_refr_layout(lv_obj_t *cont) {
  ...
  } else if (type == LV_LAYOUT_COL_L || type == LV_LAYOUT_COL_M ||
             type == LV_LAYOUT_COL_R) {
    lv_cont_layout_col(cont);
  } else if (type == LV_LAYOUT_ROW_T || type == LV_LAYOUT_ROW_M ||
...
static void lv_cont_layout_center(lv_obj_t *cont) {
  ...
  LL_READ_BACK(cont->child_ll, child) {
    ...
    lv_obj_align(child, cont, LV_ALIGN_CENTER, 0,
...
static void lv_cont_layout_pretty(lv_obj_t *cont) {
  ...
  while (child_rs != NULL) {
    ...
    do {
      ...
      if (obj_num == 0)
        child_rs =
            child_rc; /*If the first object was hidden (or too long) then set
                         the next as first */
    } while (child_rc != NULL);
...
static void lv_cont_layout_grid(lv_obj_t *cont) {
  ...
  if (obj_row > 1) {
    x_ofs = w_obj +
            (w_tot - (2 * style->body.padding.hor) - (obj_row * w_obj)) /
                (obj_row - 1);
  } else {
