...
static void lv_cont_layout_pretty(lv_obj_t* cont) {
	...
	while (child_rs != NULL) {
		...
		do {
			...
			if (obj_num == 0)
				child_rs = child_rc; /*If the first object was hidden (or too long) then set the next as first */
		} while (child_rc != NULL);
...
static void lv_cont_layout_grid(lv_obj_t* cont) {
	...
	uint16_t obj_row =
	    (w_tot - (2 * style->body.padding.hor)) / (w_obj + style->body.padding.inner); /*Obj. num. in a row*/
	lv_coord_t x_ofs;
	if (obj_row > 1) {
...
static void lv_cont_refr_autofit(lv_obj_t* cont) {
	...
	if (new_cords.x1 != LV_COORD_MAX) {
		...
		if (cont->coords.x1 != new_cords.x1 || cont->coords.y1 != new_cords.y1 || cont->coords.x2 != new_cords.x2 ||
