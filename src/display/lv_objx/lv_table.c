 ...
 * @param txt text to display in the cell. It will be copied and saved so this variable is not required after this
 * function call.
 */
		...
		for (row = 0; row < ext->row_cnt; row++) {
			...
			for (col = 0; col < ext->col_cnt; col++) {
				...
				if (ext->cell_data[cell]) {
					...
					lv_txt_get_size(&txt_size, ext->cell_data[cell] + 1, cell_style->text.font, cell_style->text.letter_space,
					                cell_style->text.line_space, lv_area_get_width(&txt_area), txt_flags);
