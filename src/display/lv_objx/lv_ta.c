...
lv_obj_t *lv_ta_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_ta);
  if (ancestor_design == NULL)
    ancestor_design = lv_obj_get_design_func(new_ta);
  if (scrl_signal == NULL)
    scrl_signal = lv_obj_get_signal_func(lv_page_get_scrl(new_ta));
  if (scrl_design == NULL)
    scrl_design = lv_obj_get_design_func(lv_page_get_scrl(new_ta));
  ...
  /*Copy an existing object*/
    ...
    lv_ta_set_style(new_ta, LV_TA_STYLE_CURSOR,
                    lv_ta_get_style(copy, LV_TA_STYLE_CURSOR));
...
void lv_ta_add_char(lv_obj_t *ta, char c) {
  ...
  if (ext->pwd_mode != 0) {
    ...
    ext->pwd_tmp = lv_mem_realloc(
        ext->pwd_tmp, strlen(ext->pwd_tmp) + 2); /*+2: the new char + \0 */
...
void lv_ta_add_text(lv_obj_t *ta, const char *txt) {
  ...
  if (ext->pwd_mode != 0) {
    ext->pwd_tmp =
        lv_mem_realloc(ext->pwd_tmp, strlen(ext->pwd_tmp) + strlen(txt) + 1);
    lv_mem_assert(ext->pwd_tmp);
...
void lv_ta_del_char(lv_obj_t *ta) {
  ...
  if (ext->pwd_mode != 0) {
    ...
    lv_txt_cut(ext->pwd_tmp, ext->cursor.pos - 1,
               lv_txt_utf8_size(label_txt[byte_pos]));
#endif
...
void lv_ta_set_cursor_pos(lv_obj_t *ta, int16_t pos) {
  ...
  if (pos > len || pos == LV_TA_CURSOR_LAST)
    pos = len;
  ...
  /*Check the bottom*/
  if (label_cords.y1 + cur_pos.y + font_h + style->body.padding.ver >
      ta_cords.y2) {
    lv_obj_set_y(label_par,
                 -(cur_pos.y - lv_obj_get_height(ta) + font_h +
                   2 * style->body.padding.ver));
  }
  ...
  if (label_cords.x1 + cur_pos.x + font_h + style->body.padding.hor >
      ta_cords.x2) {
    lv_obj_set_x(label_par,
                 -(cur_pos.x - lv_obj_get_width(ta) + font_h +
                   2 * style->body.padding.hor));
  }
...
void lv_ta_set_one_line(lv_obj_t *ta, bool en) {
  if (en != false) {
    ...
    lv_obj_set_height(
        ta,
        font_h +
            (style_ta->body.padding.ver + style_scrl->body.padding.ver) * 2);
    lv_label_set_long_mode(ext->label, LV_LABEL_LONG_EXPAND);
    lv_label_set_no_break(ext->label, true);
    lv_obj_set_pos(lv_page_get_scrl(ta), style_ta->body.padding.hor,
                   style_ta->body.padding.ver);
  } else {
    lv_ta_ext_t *ext = lv_obj_get_ext_attr(ta);
    lv_style_t *style_ta = lv_obj_get_style(ta);
...
void lv_ta_cursor_down(lv_obj_t *ta) {
  ...
  if (pos.y < lv_obj_get_height(ext->label)) {
    ...
    lv_coord_t cur_valid_x_tmp =
        ext->cursor
            .valid_x; /*Cursor position set overwrites the valid positon */
 ...
 */
static bool lv_ta_design(lv_obj_t *ta, const lv_area_t *masp,
                         lv_design_mode_t mode) {
...
static bool lv_ta_scrollable_design(lv_obj_t *scrl, const lv_area_t *mask,
  ...
  if (mode == LV_DESIGN_COVER_CHK) {
    ...
    if (ta_ext->cursor.type == LV_CURSOR_NONE ||
        (ta_ext->cursor.type & LV_CURSOR_HIDDEN) || ta_ext->cursor.state == 0 ||
    ...
    if (ta_ext->cursor.style != NULL) {
      ...
      lv_color_t clv_color_tmp =
          cur_style.text.color; /*Make letter color to cursor color*/
      cur_style.text.color =
          cur_style.body.main_color; /*In block mode the letter color will be
    ...
    if (letter_pos.x + ta_ext->label->coords.x1 + letter_w >
            ta_ext->label->coords.x2 &&
        ta_ext->one_line == 0) {
      letter_pos.x = 0;
    ...
    if (ta_ext->cursor.type == LV_CURSOR_LINE) {
      cur_area.x1 = letter_pos.x + ta_ext->label->coords.x1 +
                    cur_style.body.padding.hor - (cur_style.line.width >> 1) -
                    (cur_style.line.width & 0x1);
      cur_area.y1 =
          letter_pos.y + ta_ext->label->coords.y1 + cur_style.body.padding.ver;
      cur_area.x2 = letter_pos.x + ta_ext->label->coords.x1 +
                    cur_style.body.padding.hor + (cur_style.line.width >> 1);
      cur_area.y2 = letter_pos.y + ta_ext->label->coords.y1 +
                    cur_style.body.padding.ver + letter_h;
      lv_draw_rect(&cur_area, mask, &cur_style);
    } else if (ta_ext->cursor.type == LV_CURSOR_BLOCK) {
      cur_area.x1 =
          letter_pos.x + ta_ext->label->coords.x1 - cur_style.body.padding.hor;
      cur_area.y1 =
          letter_pos.y + ta_ext->label->coords.y1 - cur_style.body.padding.ver;
      cur_area.x2 = letter_pos.x + ta_ext->label->coords.x1 +
                    cur_style.body.padding.hor + letter_w;
      cur_area.y2 = letter_pos.y + ta_ext->label->coords.y1 +
                    cur_style.body.padding.ver + letter_h;
...
/*Get the current letter*/
      ...
      lv_draw_label(&cur_area, mask, &cur_style, letter_buf, LV_TXT_FLAG_NONE,
                    0);
      ...
      cur_area.x1 =
          letter_pos.x + ta_ext->label->coords.x1 + cur_style.body.padding.hor;
      cur_area.y1 = letter_pos.y + ta_ext->label->coords.y1 +
                    cur_style.body.padding.ver + letter_h -
                    (cur_style.line.width >> 1);
      cur_area.x2 = letter_pos.x + ta_ext->label->coords.x1 +
                    cur_style.body.padding.hor + letter_w;
...
static lv_res_t lv_ta_signal(lv_obj_t *ta, lv_signal_t sign, void *param) {
  ...
  if (sign == LV_SIGNAL_CLEANUP) {
    ...
    if (ext->label) {
      ...
      if (ext->one_line) {
        /*In one line mode refresh the Text Area height because 'vpad' can
         * modify it*/
        lv_style_t *style_label = lv_obj_get_style(ext->label);
        lv_coord_t font_h = lv_font_get_height(style_label->text.font);
        lv_obj_set_height(
            ta,
            font_h +
                (style_ta->body.padding.ver + style_scrl->body.padding.ver) *
                    2);
      } else {
        /*In not one line mode refresh the Label width because 'hpad' can modify
         * it*/
        lv_obj_set_width(ext->label,
                         lv_obj_get_width(scrl) -
                             2 * style_scrl->body.padding.hor);
        lv_obj_set_pos(
            ext->label, style_scrl->body.padding.hor,
            style_scrl->body.padding
                .ver); /*Be sure the Label is in the correct position*/
      }
      lv_label_set_text(ext->label, NULL);
    }
    ...
    if (ext->label) {
      if (lv_obj_get_width(ta) != lv_area_get_width(param) ||
        ...
        lv_obj_set_width(ext->label,
                         lv_obj_get_width(scrl) -
                             2 * style_scrl->body.padding.hor);
        lv_obj_set_pos(ext->label, style_scrl->body.padding.hor,
                       style_scrl->body.padding.ver);
    ...
    if (c == LV_GROUP_KEY_RIGHT)
      ...
      if ((c & 0x80) == 0) { /*ASCII*/
        ...
        for (i = 0; i < 4; i++) {
          if (p[0] == 0)
            p++; /*Ignore leading zeros (they were in the end originally)*/
        }
        lv_ta_add_text(ta, p);
      }
#else
...
static lv_res_t lv_ta_scrollable_signal(lv_obj_t *scrl, lv_signal_t sign,
  ...
  if (sign == LV_SIGNAL_REFR_EXT_SIZE) {
    ...
    lv_coord_t font_h = lv_font_get_height(style_label->text.font);
    scrl->ext_size =
        LV_MATH_MAX(scrl->ext_size, style_label->text.line_space + font_h);
  }
