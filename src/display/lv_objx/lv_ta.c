...
void lv_ta_set_cursor_pos(lv_obj_t* ta, int16_t pos) {
	...
	if (label_cords.y1 + cur_pos.y + font_h + style->body.padding.ver > ta_cords.y2) {
		lv_obj_set_y(label_par, -(cur_pos.y - lv_obj_get_height(ta) + font_h + 2 * style->body.padding.ver));
	}
	...
	if (label_cords.x1 + cur_pos.x + font_h + style->body.padding.hor > ta_cords.x2) {
		lv_obj_set_x(label_par, -(cur_pos.x - lv_obj_get_width(ta) + font_h + 2 * style->body.padding.hor));
	}
		...
		if (ext->label) {
			...
			if (ext->one_line) {
				...
				lv_obj_set_pos(ext->label, style_scrl->body.padding.hor,
				               style_scrl->body.padding.ver); /*Be sure the Label is in the correct position*/
			}
			lv_label_set_text(ext->label, NULL);
		}
...
static void refr_cursor_area(lv_obj_t* ta) {
	...
	if (ext->cursor.type == LV_CURSOR_LINE) {
		cur_area.x1 =
		    letter_pos.x + cur_style.body.padding.hor - (cur_style.line.width >> 1) - (cur_style.line.width & 0x1);
		...
		cur_area.x2 = letter_pos.x + cur_style.body.padding.hor + letter_w;
		cur_area.y2 = letter_pos.y + cur_style.body.padding.ver + letter_h + (cur_style.line.width >> 1) +
