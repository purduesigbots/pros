...
lv_obj_t *lv_bar_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_bar);
  if (ancestor_design_f == NULL)
    ancestor_design_f = lv_obj_get_design_func(new_bar);
...
static bool lv_bar_design(lv_obj_t *bar, const lv_area_t *mask,
  ...
  if (mode == LV_DESIGN_COVER_CHK) {
    ...
    if (w >= h) {
      indic_area.x2 =
          (int32_t)((int32_t)w * (ext->cur_value - ext->min_value)) /
          (ext->max_value - ext->min_value);
      indic_area.x2 += indic_area.x1;
    } else {
      indic_area.y1 =
          (int32_t)((int32_t)h * (ext->cur_value - ext->min_value)) /
          (ext->max_value - ext->min_value);
