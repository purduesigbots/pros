...
static lv_res_t lv_lmeter_signal(lv_obj_t *lmeter, lv_signal_t sign,
                                 void *param);
...
lv_obj_t *lv_lmeter_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_lmeter);

  /*Allocate the line meter type specific extended data*/
  lv_lmeter_ext_t *ext =
      lv_obj_allocate_ext_attr(new_lmeter, sizeof(lv_lmeter_ext_t));
  lv_mem_assert(ext);
...
void lv_lmeter_set_value(lv_obj_t *lmeter, int16_t value) {
  ...
  ext->cur_value =
      ext->cur_value < ext->min_value ? ext->min_value : ext->cur_value;
...
static bool lv_lmeter_design(lv_obj_t *lmeter, const lv_area_t *mask,
  ...
  /*Draw the object*/
    ...
    int16_t level =
        (int32_t)((int32_t)(ext->cur_value - ext->min_value) * ext->line_cnt) /
    ...
    for (i = 0; i < ext->line_cnt; i++) {
      ...
      lv_coord_t y_out =
          (int32_t)((int32_t)lv_trigo_sin(angle) * r_out) >> LV_TRIGO_SHIFT;
      lv_coord_t x_out = (int32_t)((int32_t)lv_trigo_sin(angle + 90) * r_out) >>
                         LV_TRIGO_SHIFT;
      lv_coord_t y_in =
          (int32_t)((int32_t)lv_trigo_sin(angle) * r_in) >> LV_TRIGO_SHIFT;
      lv_coord_t x_in =
          (int32_t)((int32_t)lv_trigo_sin(angle + 90) * r_in) >> LV_TRIGO_SHIFT;
      ...
      if (i >= level)
        ...
        style_tmp.line.color =
            lv_color_mix(style->body.grad_color, style->body.main_color,
                         (255 * i) / ext->line_cnt);
      }
