...
static lv_res_t lv_tileview_scrl_signal(lv_obj_t* scrl, lv_signal_t sign, void* param) {
	...
	if (sign == LV_SIGNAL_CORD_CHG) {
		...
		if (indev) {
			...
			if (ext->drag_ver == 0 && ((ext->drag_right_en && indev->proc.drag_sum.x <= -LV_INDEV_DRAG_LIMIT) ||
			...
			if (ext->drag_hor == 0 && ((ext->drag_bottom_en && indev->proc.drag_sum.y <= -LV_INDEV_DRAG_LIMIT) ||
			...
			if (ext->drag_top_en == 0) {
				if (y > -(ext->act_id.y * h) && indev->proc.vect.y > 0 && ext->drag_hor == 0) {
					if (ext->page.edge_flash.enabled && ext->page.edge_flash.left_ip == 0 && ext->page.edge_flash.right_ip == 0 &&
...
static lv_res_t element_signal_func(lv_obj_t* element, lv_signal_t sign, void* param) {
	...
	if (sign == LV_SIGNAL_PRESSED) {
		...
		while (tileview) {
			if (lv_obj_get_signal_func(tileview) != lv_tileview_signal)
				tileview = lv_obj_get_parent(tileview);
			else
				break;
		}
