 ...
 * @param copy pointer to a window object, if not NULL then the new object will
 * be copied from it
...
lv_obj_t *lv_win_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_win);
  ...
  if (copy == NULL) {
    ...
    if (th) {
      ...
      lv_win_set_style(new_win, LV_WIN_STYLE_CONTENT_SCRL,
                       th->win.content.scrl);
 ...
 * @return always LV_ACTION_RES_INV because the button is deleted with the
 * window
 */
      ...
      while (child != NULL) {
        if (lv_obj_is_protected(child, LV_PROTECT_PARENT) == false) {
          ...
          child = lv_obj_get_child(
              win, child); /*Get the next child before move this*/
...
static void lv_win_realign(lv_obj_t *win) {
  ...
  while (btn != NULL) {
    ...
    if (first_btn) {
      lv_obj_align(btn, ext->header, LV_ALIGN_IN_RIGHT_MID,
                   -header_style->body.padding.hor, 0);
      first_btn = false;
    } else {
  ...
  lv_obj_align(ext->title, NULL, LV_ALIGN_IN_LEFT_MID,
               ext->style_header->body.padding.hor, 0);
