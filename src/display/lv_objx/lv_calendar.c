 ...
 * @param calendar pointer to a calendar object
 * @param today pointer to an `lv_calendar_date_t` variable containing the date of today. The value will be saved it can
 ...
 * @param calendar pointer to a calendar object
 * @param showed pointer to an `lv_calendar_date_t` variable containing the date to show. The value will be saved it can
 * be local variable too.
 */
 ...
 * @param calendar pointer to a calendar object
 * @param highlighted pointer to an `lv_calendar_date_t` array containing the dates. ONLY A POINTER WILL BE SAVED! CAN'T
 * BE LOCAL ARRAY.
...
static bool calculate_touched_day(lv_obj_t* calendar, const lv_point_t* touched_point) {
	...
	days_area.y1 =
	    calendar->coords.y1 + get_header_height(calendar) + get_day_names_height(calendar) - style_bg->body.padding.ver;
...
static void draw_days(lv_obj_t* calendar, const lv_area_t* mask) {
	...
	lv_opa_t opa_scale = lv_obj_get_opa_scale(calendar);
	label_area.y1 = calendar->coords.y1 + get_header_height(calendar) + ext->style_day_names->body.padding.ver +
	                lv_font_get_height(ext->style_day_names->text.font) + ext->style_day_names->body.padding.ver;
