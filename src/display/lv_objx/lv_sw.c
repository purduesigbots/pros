...
#if USE_LV_SLIDER == 0
#error                                                                         \
    "lv_sw: lv_slider is required. Enable it in lv_conf.h (USE_LV_SLIDER  1) "
#endif
...
lv_obj_t *lv_sw_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_sw);
...
void lv_sw_set_style(lv_obj_t *sw, lv_sw_style_t type, lv_style_t *style) {
  ...
  case LV_SW_STYLE_KNOB_OFF:
    ...
    if (lv_sw_get_state(sw) == 0)
      lv_slider_set_style(sw, LV_SLIDER_STYLE_KNOB, style);
    break;
...
static lv_res_t lv_sw_signal(lv_obj_t *sw, lv_signal_t sign, void *param) {
  ...
  /*Do not let the slider to call the callback. The Switch will do it if
   * required*/
