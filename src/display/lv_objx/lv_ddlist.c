	...
	/*Post draw when the children are drawn*/
		...
		if (ext->opened) {
			...
			if (area_ok) {
				...
				lv_draw_label(&ext->label->coords, &mask_sel, &new_style, opa_scale, lv_label_get_text(ext->label), flag, NULL);
			}
		}
		...
		/*Add a down symbol in ddlist when closed*/
			...
			if (ext->draw_arrow) {
				...
				if (area_ok) {
					lv_draw_label(&area_arrow, &mask_arrow, &new_style, opa_scale, SYMBOL_DOWN, LV_TXT_FLAG_NONE,
					              NULL); /*Use a down arrow in ddlist, you can replace it with your custom symbol*/
				}
			}
		}
	...
	if (sign == LV_SIGNAL_STYLE_CHG) {
		...
		if (indev_type == LV_INDEV_TYPE_ENCODER) {
			...
			if (!ext->opened) {
				ext->opened = true;
				ext->sel_opt_id_ori =
				    ext->sel_opt_id; /*Save the current value. Used to revert this state if ENER wont't be pressed*/
				lv_ddlist_refr_size(ddlist, true);
			}
		}
#endif
...
static void lv_ddlist_refr_size(lv_obj_t* ddlist, bool anim_en) {
	...
	if (ext->opened) { /*Open the list*/
		if (ext->fix_height == 0)
			new_height = lv_obj_get_height(lv_page_get_scrl(ddlist)) + 2 * style->body.padding.ver;
		else
...
#if USE_LV_ANIMATION
		lv_anim_del(ddlist,
		            (lv_anim_fp_t)lv_obj_set_height); /*If an animation is in progress then it will overwrite this changes*/
	} else {
		lv_anim_t a;
		a.var = ddlist;
