...
static lv_res_t lv_ddlist_signal(lv_obj_t *ddlist, lv_signal_t sign,
                                 void *param);
static lv_res_t lv_ddlist_scrl_signal(lv_obj_t *scrl, lv_signal_t sign,
                                      void *param);
...
lv_obj_t *lv_ddlist_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_ddlist);
  ...
  if (ancestor_design == NULL)
    ancestor_design = lv_obj_get_design_func(new_ddlist);

  /*Allocate the drop down list type specific extended data*/
  lv_ddlist_ext_t *ext =
      lv_obj_allocate_ext_attr(new_ddlist, sizeof(lv_ddlist_ext_t));
  lv_mem_assert(ext);
...
static lv_res_t lv_ddlist_scrl_signal(lv_obj_t *scrl, lv_signal_t sign,
  ...
  if (sign == LV_SIGNAL_REFR_EXT_SIZE) {
     ...
     * In this way by dragging the scrollable part the wider rectangle area can
     * be redrawn too*/
    lv_style_t *style = lv_ddlist_get_style(ddlist, LV_DDLIST_STYLE_BG);
    if (scrl->ext_size < style->body.padding.hor)
      scrl->ext_size = style->body.padding.hor;
  } else if (sign == LV_SIGNAL_CLEANUP) {
    lv_ddlist_ext_t *ext = lv_obj_get_ext_attr(ddlist);
 ...
 * @param ddlist pointer to a drop down list object
 * @return LV_ACTION_RES_INV if the ddlist it deleted in the user callback else
 * LV_ACTION_RES_OK
 */
static lv_res_t lv_ddlist_release_action(lv_obj_t *ddlist) {
  lv_ddlist_ext_t *ext = lv_obj_get_ext_attr(ddlist);
 ...
 * @param ddlist pointer to a drop down list object
 * @param anim_en Change the size (open/close) with or without animation
 * (true/false)
 */
static void lv_ddlist_refr_size(lv_obj_t *ddlist, bool anim_en) {
#if USE_LV_ANIMATION == 0
  ...
  if (ext->opened) { /*Open the list*/
    if (ext->fix_height == 0)
      new_height = lv_obj_get_height(lv_page_get_scrl(ddlist)) +
