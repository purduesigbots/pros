...
static bool lv_btnm_design(lv_obj_t* btnm, const lv_area_t* mask, lv_design_mode_t mode) {
	...
	/*Draw the object*/
		...
		for (btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++) {
			...
			lv_point_t txt_size;
			lv_txt_get_size(&txt_size, ext->map_p[txt_i], font, btn_style->text.letter_space, btn_style->text.line_space,
	...
	if (sign == LV_SIGNAL_CLEANUP) {
		...
		if (ext->action && ext->btn_id_pr != LV_BTNM_PR_NONE) {
			...
			if (txt_i != LV_BTNM_PR_NONE) {
				if (button_is_repeat_disabled(ext->map_p[txt_i]) == false && button_is_inactive(ext->map_p[txt_i]) == false) {
		...
		if (ext->btn_id_pr != LV_BTNM_PR_NONE) {
			uint16_t txt_i = get_button_text(btnm, ext->btn_id_pr);
			if (button_is_inactive(ext->map_p[txt_i]) == false &&
			    txt_i != LV_BTNM_PR_NONE) { /*Ignore the inactive buttons anf click between the buttons*/
...
static bool button_is_hidden(const char* btn_str) {
	...
	if (((btn_str[0] & LV_BTNM_CTRL_MASK) == LV_BTNM_CTRL_CODE) && (btn_str[0] & LV_BTNM_HIDE_MASK)) {
		return true;
	}
...
static bool button_is_repeat_disabled(const char* btn_str) {
	/*If control byte presents and hidden bit is '1' then the button is hidden*/
	if (((btn_str[0] & LV_BTNM_CTRL_MASK) == LV_BTNM_CTRL_CODE) && (btn_str[0] & LV_BTNM_REPEAT_DISABLE_MASK)) {
		return true;
	}
...
static bool button_is_inactive(const char* btn_str) {
	/*If control byte presents and hidden bit is '1' then the button is hidden*/
	if (((btn_str[0] & LV_BTNM_CTRL_MASK) == LV_BTNM_CTRL_CODE) && (btn_str[0] & LV_BTNM_INACTIVE_MASK)) {
		return true;
	}
