...
lv_obj_t *lv_btnm_create(lv_obj_t *par, lv_obj_t *copy) {
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_btnm);

  /*Allocate the object type specific extended data*/
  lv_btnm_ext_t *ext =
      lv_obj_allocate_ext_attr(new_btnm, sizeof(lv_btnm_ext_t));
  lv_mem_assert(ext);
  ext->btn_cnt = 0;
  ...
  if (ancestor_design_f == NULL)
    ancestor_design_f = lv_obj_get_design_func(new_btnm);
...
void lv_btnm_set_map(lv_obj_t *btnm, const char **map) {
  ...
  while (1) {
    ...
    if (btn_cnt != 0) {
      ...
      lv_coord_t all_unit_w =
          max_w - ((btn_cnt - 1) * style_bg->body.padding.inner);
      ...
      for (i = 0; i < btn_cnt; i++) {
         ...
         * do this two operations but the multiply first to divide a greater
         * number */
        act_unit_w = (all_unit_w * get_button_width(map_p_tmp[i])) / unit_cnt;
        act_unit_w--; /*-1 because e.g. width = 100 means 101 pixels (0..100)*/

        /*Always recalculate act_x because of rounding errors */
        act_x = (unit_act_cnt * all_unit_w) / unit_cnt +
                i * style_bg->body.padding.inner + style_bg->body.padding.hor;

        /* Set the button's area.
         * If inner padding is zero then use the prev. button x2 as x1 to avoid
         * rounding errors*/
        if (style_bg->body.padding.inner == 0 &&
            act_x != style_bg->body.padding.hor) {
          lv_area_set(&ext->button_areas[btn_i],
                      ext->button_areas[btn_i - 1].x2, act_y,
    ...
    /*If no vertical padding then make sure the last row is at the bottom of
     * 'btnm'*/
 ...
 */
void lv_btnm_set_style(lv_obj_t *btnm, lv_btnm_style_t type,
                       lv_style_t *style) {
  lv_btnm_ext_t *ext = lv_obj_get_ext_attr(btnm);
...
static bool lv_btnm_design(lv_obj_t *btnm, const lv_area_t *mask,
  ...
  /*Draw the object*/
    ...
    for (btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++) {
      ...
      /*Skip hidden buttons*/
      if (button_is_hidden(ext->map_p[txt_i]))
        continue;
      ...
      /*Load the style*/
      if (button_is_inactive(ext->map_p[txt_i]))
        btn_style = lv_btnm_get_style(btnm, LV_BTNM_STYLE_BTN_INA);
      else if (btn_i != ext->btn_id_pr && btn_i != ext->btn_id_tgl)
        ...
        btn_style = lv_btnm_get_style(
            btnm,
            LV_BTNM_STYLE_BTN_REL); /*Not possible option, just to be sure*/

      /*On the right buttons clear the border if only right borders are drawn*/
      if (ext->map_p[txt_i + 1][0] == '\0' ||
          ext->map_p[txt_i + 1][0] == '\n') {
      ...
      if (btn_style->glass)
        btn_style = bg_style;
      lv_draw_label(&area_tmp, mask, btn_style, ext->map_p[txt_i],
  ...
  if (sign == LV_SIGNAL_CLEANUP) {
    ...
    if (c == LV_GROUP_KEY_RIGHT) {
      if (ext->btn_id_pr == LV_BTNM_PR_NONE)
        ...
        ext->btn_id_pr++;
      if (ext->btn_id_pr >= ext->btn_cnt - 1)
        ext->btn_id_pr = ext->btn_cnt - 1;
      lv_obj_invalidate(btnm);
      ...
      if (ext->btn_id_pr == LV_BTNM_PR_NONE) {
        ...
        for (area_below = ext->btn_id_pr; area_below < ext->btn_cnt;
          ...
          if (ext->button_areas[area_below].y1 >
              ...
              pr_center <=
                  ext->button_areas[area_below].x2 + style->body.padding.hor) {
            break;
          }
        }
      ...
      if (ext->btn_id_pr == LV_BTNM_PR_NONE) {
        ...
        for (area_above = ext->btn_id_pr; area_above > 0; area_above--) {
          if (ext->button_areas[area_above].y1 <
                  ext->button_areas[ext->btn_id_pr].y1 &&
              pr_center >=
                  ext->button_areas[area_above].x1 - style->body.padding.hor &&
 ...
 * @param btn_str The descriptor string of a button. E.g. "apple" or
 * "\004banana"
