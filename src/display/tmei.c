...
void register_touch_callback(touch_event_cb_fn_t cb,
  ...
  case E_TOUCH_EVENT_RELEASE:
    linked_list_prepend_func(_touch_event_release_handler_list,
                             (generic_fn_t)cb);
    break;
  ...
  case E_TOUCH_EVENT_PRESS_AND_HOLD:
    linked_list_prepend_func(_touch_event_press_auto_handler_list,
                             (generic_fn_t)cb);
    break;
  }
}
...
void unregister_touch_callback(touch_event_cb_fn_t cb,
  ...
  switch (event_type) {
  case E_TOUCH_EVENT_RELEASE:
    linked_list_remove_func(_touch_event_release_handler_list,
                            (generic_fn_t)cb);
    break;
  case E_TOUCH_EVENT_PRESS:
    ...
    break;
  case E_TOUCH_EVENT_PRESS_AND_HOLD:
    linked_list_remove_func(_touch_event_press_auto_handler_list,
                            (generic_fn_t)cb);
    break;
  }
}
...
void _handle_cb(ll_node_s_t *current, void *data) {
  ((touch_event_cb_fn_t)(current->payload.func))(
      ((touch_event_position_data_s_t *)(data))->x,
      ((touch_event_position_data_s_t *)(data))->y);
}
static inline bool _touch_status_equivalent(V5_TouchStatus x,
                                            V5_TouchStatus y) {
  return (x.lastEvent == y.lastEvent) && (x.lastXpos == y.lastXpos) &&
         (x.lastYpos == y.lastYpos);
}
void _touch_handle_task(void *ignore) {
  ...
  while (true) {
    ...
    if (!_touch_status_equivalent(current, last)) {
      touch_event_position_data_s_t event_data = {.x = current.lastXpos,
                                                  .y = current.lastYpos};
      switch (current.lastEvent) {
      case E_TOUCH_EVENT_RELEASE:
        linked_list_foreach(_touch_event_release_handler_list, _handle_cb,
                            (void *)&event_data);
        break;
      case E_TOUCH_EVENT_PRESS:
        linked_list_foreach(_touch_event_press_handler_list, _handle_cb,
                            (void *)&event_data);
        break;
      case E_TOUCH_EVENT_PRESS_AND_HOLD:
...
void graphical_context_daemon_initialize(void) {
  ...
  touch_handle_task =
      task_create_static(_touch_handle_task, NULL, TASK_PRIORITY_MAX - 3,
                         TASK_STACK_DEPTH_DEFAULT, "PROS TMEI Touch Handler",
                         touch_handle_task_stack, &touch_handle_task_buffer);
}
