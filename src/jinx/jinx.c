...
// Number of bytes reserved for messagepack overhead (3 bytes) and for the JINX
// message type (1)
...
static struct gid_metadata jinx_gid_metadata = {
    .bitmap = gid_bitmap,
    .max = JINX_GID_MAX,
...
static void data_flush() {
  lcd_print(0, "%x %u", *(uint32_t *)(data_buffer + 1),
    ...
    if (since_last_variable_push >= VARIABLE_TIMEOUT) {
      for (jinx_track_s_t *curr = track_buff; curr != NULL; curr = curr->next) {
        if (curr->contains_func) {
          void *buff =
              kmalloc(((struct jinx_variable *)(curr->jinx_var))->value_size);
          if (curr->data_func.fill_func_data.fill_func(
                  buff, curr->data_func.fill_func_data.args)) {
      ...
      register uint32_t next_variable =
          VARIABLE_TIMEOUT - since_last_variable_push;
...
static int handle_cwpack_buffer_overflow(struct cw_pack_context *ctx,
                                         unsigned long add) {
...
void jinx_init(void) {
  ...
  jinx_daemon_task = task_create_static(
      jinx_task, NULL, TASK_PRIORITY_MIN + 2, TASK_STACK_DEPTH_DEFAULT,
...
jinx_var_t jinx_create_variable(uint16_t value_size, char *value_name,
                                char *value_fmt,
                                jinx_var_extra_args_t *extra_args) {
  static struct cw_pack_context cwp_ctx;
  cw_pack_context_init(&cwp_ctx, kmalloc(0x20), 0x20,
                       &handle_cwpack_buffer_overflow);
  ...
  if (extra_args != NULL) {
    ...
    for (int i = 0; i < extra_args->n_subelements; i++) {
      cw_pack_str(&cwp_ctx, (extra_args->subelements)[i],
                  strlen((extra_args->subelements)[i]));
    }
...
jinx_track_s_t *jinx_track_variable_fptr(jinx_var_t jinx_value,
                                         bool (*fill_func)(void *buff,
                                                           void *args),
                                         void *args_to_pass) {
  jinx_track_s_t *new_var = kmalloc(sizeof(*new_var));
  new_var->jinx_var = jinx_value;
