...
int32_t ext_adi_pin_mode(uint8_t smart_port, uint8_t adi_port, uint8_t mode) {
	switch (mode) {
		case INPUT:
			ext_adi_port_set_config(smart_port - 1, adi_port, E_ADI_DIGITAL_IN);
			break;
		case OUTPUT:
			ext_adi_port_set_config(smart_port - 1, adi_port, E_ADI_DIGITAL_OUT);
			break;
		case INPUT_ANALOG:
			ext_adi_port_set_config(smart_port - 1, adi_port, E_ADI_ANALOG_IN);
			break;
		case OUTPUT_ANALOG:
			ext_adi_port_set_config(smart_port - 1, adi_port, E_ADI_ANALOG_OUT);
			break;
		default:
			errno = EINVAL;
			return PROS_ERR;
...
ext_adi_encoder_t ext_adi_encoder_init(uint8_t smart_port, uint8_t adi_port_top, uint8_t adi_port_bottom, const bool reverse) {
	...
	return_port(smart_port - 1, port + 1);
}
...
int32_t ext_adi_ultrasonic_get(ext_adi_ultrasonic_t ult) {
	uint8_t smart_port;
	uint8_t adi_port;
	get_ports(ult, smart_port - 1, adi_port);
	transform_adi_port(adi_port);
	claim_port_i(smart_port - 1, E_DEVICE_ADI);
...
int32_t _ext_adi_gyro_get(ext_adi_gyro_t gyro, double* out) {
	uint8_t smart_port;
	uint8_t adi_port;
	get_ports(gyro, smart_port - 1, adi_port);
	transform_adi_port(adi_port);
	claim_port_i(smart_port - 1, E_DEVICE_ADI);
	validate_type(device, smart_port - 1, E_ADI_LEGACY_GYRO);
...
double ext_adi_gyro_get(ext_adi_gyro_t gyro) {
	uint8_t smart_port;
	uint8_t adi_port;
	get_smart_port(gyro, smart_port - 1);
	double rtn;
