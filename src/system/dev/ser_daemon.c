...
void print_small_banner(void) {
  ...
  iprintf(short_banner, PROS_VERSION_STRING, uptime / 1000, uptime % 1000,
          _PROS_COMPILE_TIMESTAMP, _PROS_COMPILE_DIRECTORY);
}
...
void print_large_banner(void) {
  ...
  *sys_ver = vexSystemVersion();
  uint32_t uptime = millis();
  iprintf(large_banner, PROS_VERSION_STRING, version[3], version[2], version[1],
          version[0], uptime / 1000, uptime % 1000, _PROS_COMPILE_TIMESTAMP,
...
/******************************************************************************/
#define INP_BUFFER_SIZE                                                        \
  0x1000 // 4KB... which is larger than VEX's output buffer -_-
...
static inline void inp_buffer_initialize() {
  inp_stream =
      stream_buf_create_static(INP_BUFFER_SIZE, 1, inp_buffer, &inp_stream_buf);
}
...
int32_t inp_buffer_available() { return stream_buf_get_used(inp_stream); }
...
void ser_initialize(void) {
  ...
  task_create_static(ser_daemon_task, NULL, TASK_PRIORITY_MIN + 1,
                     TASK_STACK_DEPTH_MIN, "Serial Daemon (PROS)",
