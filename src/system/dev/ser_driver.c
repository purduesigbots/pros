...
void ser_output_flush(void) {
  size_t len = stream_buf_recv(write_stream, write_scratch_buf,
                               vexSerialWriteFree(1), 0);
...
/******************************************************************************/
int ser_read_r(struct _reent *r, void *const arg, uint8_t *buffer,
               const size_t len) {
...
int ser_write_r(struct _reent *r, void *const arg, const uint8_t *buf,
                const size_t len) {
  const ser_file_s_t file = *(ser_file_s_t *)arg;

  if (!list_contains(guaranteed_delivery_streams,
                     guaranteed_delivery_streams_size, file.stream_id) &&
  ...
  if (ser_driver_runtime_config &= E_COBS_ENABLED) {
    ...
    if (!mutex_take(write_mtx,
                    (file.flags & E_NOBLK_WRITE) ? 0 : TIMEOUT_MAX)) {
      r->_errno = EACCES;
      return 0;
    }
    ...
    bool ret =
        ser_output_write(cobs_buf, cobs_len + 1, file.flags & E_NOBLK_WRITE);
...
int32_t serctl(const uint32_t action, void *const extra_arg) {
  ...
  case SERCTL_ACTIVATE:
    if (!list_contains(guaranteed_delivery_streams,
                       guaranteed_delivery_streams_size, (uint32_t)extra_arg)) {
      // grep DEBUG point
