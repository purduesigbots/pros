...
typedef struct usd_file_arg { FIL *ifi_fptr; } usd_file_arg_t;

static const int FRESULTMAP[] = {0,       EIO,    EINVAL, EBUSY,
                                 ENOENT,  ENOENT, EINVAL, EACCES, // FR_DENIED
                                 EEXIST,  EINVAL, EROFS,  ENXIO,
                                 ENOBUFS, ENXIO,  EIO,    EACCES, // FR_LOCKED
...
/******************************************************************************/
int usd_read_r(struct _reent *r, void *const arg, uint8_t *buffer,
               const size_t len) {
  usd_file_arg_t *file_arg = (usd_file_arg_t *)arg;
  // TODO: mutex here. Global or file lock?
  int32_t result =
      vexFileRead((char *)buffer, sizeof(*buffer), len, file_arg->ifi_fptr);
  return result;
}

int usd_write_r(struct _reent *r, void *const arg, const uint8_t *buf,
                const size_t len) {
  usd_file_arg_t *file_arg = (usd_file_arg_t *)arg;
  // TODO: mutex here. Global or file lock?
  int32_t result =
      vexFileWrite((char *)buf, sizeof(*buf), len, file_arg->ifi_fptr);
  return result;
}
