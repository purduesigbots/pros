...
typedef struct dev_file_arg { uint32_t port; } dev_file_arg_t;

/******************************************************************************/
/**                         newlib driver functions                          **/
/******************************************************************************/
int dev_read_r(struct _reent *r, void *const arg, uint8_t *buffer,
               const size_t len) {
  ...
  // check flags
  int32_t result =
      vexDeviceGenericSerialReceive(device->device_info, buffer, len);
  return_port(port - 1, result);
}

int dev_write_r(struct _reent *r, void *const arg, const uint8_t *buf,
                const size_t len) {
  dev_file_arg_t *file_arg = (dev_file_arg_t *)arg;
  uint32_t port = file_arg->port;
  claim_port(port - 1, E_DEVICE_GENERIC);
  int32_t result = vexDeviceGenericSerialTransmit(device->device_info,
                                                  (uint8_t *)buf, (int32_t)len);
  vexDeviceGenericSerialFlush(device->device_info);
  return_port(port - 1, result);
}

int dev_close_r(struct _reent *r, void *const arg) { return 0; }
