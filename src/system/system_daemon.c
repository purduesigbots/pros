...
// libv5rts doesn't expose the symbol but gives us the prototype or return
// values (woops)
...
static void _system_daemon_task(void *ign) {
  ...
  // monitoring mode
  competition_task =
      task_create_static(_initialize_task, NULL, TASK_PRIORITY_DEFAULT,
                         TASK_STACK_DEPTH_DEFAULT, "User Initialization (PROS)",
  ...
  while (1) {
    ...
    if (unlikely(status != competition_get_status())) {
      ...
      // competition initialize runs only when entering disabled and when we
      // weren't previously
      // connected to field control
      if (new_state == E_STATE_DISABLED &&
          !(old_status & COMPETITION_CONNECTED)) {
      ...
      if (task_state == E_TASK_STATE_READY ||
          task_state == E_TASK_STATE_BLOCKED ||
          task_state == E_TASK_STATE_SUSPENDED) {
        task_delete(competition_task);
      }

      competition_task =
          task_create_static(task_fns[new_state], NULL, TASK_PRIORITY_DEFAULT,
...
// which calls the user's autonomous()
// implement w/C++ linkage: sysd starts _autonomous_task calls autonomous()
// defined here which calls cpp_autonomous
// which calls the user's C++ linkage autonomous()
// implement no autonomous: see above, but cpp_autonomous implements a stub C++
// autonomous()

// Our weak functions call C++ links of these functions, allowing users to only
// optionally extern "C" the task functions
...
__attribute__((weak)) void autonomous() { cpp_autonomous(); }
__attribute__((weak)) void initialize() { cpp_initialize(); }
__attribute__((weak)) void opcontrol() { cpp_opcontrol(); }
__attribute__((weak)) void disabled() { cpp_disabled(); }
...
static void _disabled_task(void *ign) { disabled(); }
